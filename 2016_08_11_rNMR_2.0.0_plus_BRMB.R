...
pe <- function(table){
	...
	onView <- function(){
		...
		setGraphics(w1Range=w1Range, w2Range=w2Range, refresh.graphics=TRUE)
	}
...
re <- function(table){
	...
	onView <- function(){
		...
		if (roiTable$nDim[usrSel] != nDim && nDim == 1 )
			tusr[3:4] <- c(fileFolder[[current]]$file.par$min_intensity, 
					fileFolder[[current]]$file.par$max_intensity)
		setGraphics(usr=tusr, refresh.graphics=TRUE)
	}
	tkbutton(dlg, command=onView)
...
roi <- function(){
	...
	subRoiButton <- ttkbutton(subFrame, text='ROIs', width=8, command=onSubSel)
	...
	subRoisButton <- ttkcheckbutton(winFrame, variable=subVal, command=onSubDisp,
			text='Subplot')
...
parseAcqus <- function(inDir, params){ 
	...
	for (i in seq_along(files)){
		...
		for (paramSep in c('= ', '=', ' =', ' = ')){
			splitText <- strsplit(readLines(files[i], warn=FALSE), paramSep)
...
parseProcpar <- function(procpar, params, idn='dn'){
	...
	varPar <- strsplit(readLines(procpar, warn=FALSE), ' ')
...
ascii2rnmr <- function(inFile, outFile, nuc, freq=NA, drift=FALSE, 
	...
	line1 <- suppressWarnings(readLines(inFile, 1, warn=FALSE))
	...
	if (length(line1) == (nDim + 1)){
		...
		if (any(is.na(suppressWarnings(as.numeric(line1))))){
			line1 <- strsplit(suppressWarnings(readLines(inFile, 2, warn=FALSE))[2],
					sep)[[1]]
			skip <- 1
		}else
			skip <- 0
...
############################ auto_assign.R ####################################
## Read in UCSF format data for a spectrum in fileFolder
## inFolder - list; data and file parameters for spectra.  This should match the
##	output format of ucsf2D()
##Note - this function is a faster replacement for ucsf2D()
ucsfData <- function( inFolder = fileFolder[[wc()]] ){
	
	## Redirect for rsd files
	if( !is.null(inFolder$file.par$block_upfield_ppms) )
		return(rsd2D( inFolder$file.par$file.name, file.par =  inFolder$file.par))
	
	## Find total usable tiles if none exists
	if(is.null(inFolder$graphics.par$tiles))
		inFolder <- findTiles(in.folder = inFolder)
	tiles <- sort(inFolder$graphics.par$tiles[inFolder$graphics.par$tiles >= 0])
	
	## Define some local variables
	bs <- inFolder$file.par$block_size
	ms <- inFolder$file.par$matrix_size
	uf <- inFolder$file.par$upfield_ppm
	df <- inFolder$file.par$downfield_ppm
	endFormat <- inFolder$file.par$endian
	binLoc <- inFolder$file.par$binary_location
	
	## Setup output matrix
	w1TileNum <- ceiling(ms[1] / bs[1] )
	w2TileNum <- ceiling(ms[2] / bs[2] )
	outData <- list(file.par = inFolder$file.par,
			w1 = seq(uf[1], df[1], length.out = ms[1]),
			w2 = seq(uf[2], df[2], length.out = ms[2]),
			data = matrix( 0, nrow = w2TileNum * bs[2], ncol = w1TileNum * bs[1]))
	
	##Read binary data for each tile
	con <- myFile(inFolder$file.par$file.name, "rb")
	for(i in tiles){ 
		...
		## Read data for current tile
		seek(con, bs[1] * bs[2] * 4 * i + binLoc, origin = 'start')
		tileData <- matrix((readBin(con, size=4, what='double',
									endian = endFormat, n=(bs[1] * bs[2]))), ncol=bs[1])
		outData$data[w2B, w1B] <- tileData
	}	
	outData$data <- outData$data[1:ms[2], 1:ms[1]]
...
## Returns updated file parameters for a spectrum based on data provided 
## inFolder - list; data and file parameters for input spectrum.  This should 
##	match the	output format of ucsf2D()
## Arguments match fields found in the file.par section of fileFolder and may
##	be used to override the output values of provided fields
parMod <- function( inFolder, file.name, file.size, date.modified, axis, 
		nucleus, matrix_size, block_size, upfield_ppm, downfield_ppm, 
		spectrum_width_Hz, transmitter_MHz, center_ppm, binary_location, 
		endian, number_dimensions, noise_est, min_intensity, 
		max_intensity, zero_offset, user_title) {
	
	outPar <- inFolder$file.par
	
	## Update the date/time
	outPar$date.modified <- Sys.time()
	
	## Update file.par based on data provided
	if( !is.null(inFolder$data) ){
		
		## Remove tiles from data
		if (outPar$number_dimensions == 2)
			outPar$matrix_size <- outPar$block_size <- rev( dim(inFolder$data))
		else
			outPar$matrix_size <- outPar$block_size <- length(inFolder$data)
		
		## General method for calculating file size	
		outSize <- ((outPar$matrix_size - 1) %/% outPar$block_size + 1) * 
				outPar$block_size
		outPar$file.size <- outSize[1] * outSize[2] * 4 + 436
		
		## Update upfield, downfield, and center shifts
		outPar$upfield_ppm <- c(inFolder$w1[1], inFolder$w2[1])
		outPar$downfield_ppm <- c(inFolder$w1[length(inFolder$w1)], 
				inFolder$w2[length(inFolder$w2)])
		outPar$center_ppm <- outPar$downfield_ppm - ((outPar$downfield_ppm - 
						outPar$upfield_ppm) / 2)
		
		## Update file range
		fileRange <- fivenum( inFolder$data )
		outPar$noise_est <- (diff(fileRange[c(2, 4)]) / 2) / .674
		if( outPar$noise_est == 0 || is.na(outPar$noise_est))
			outPar$noise_est <- 0.1
		outPar$min_intensity <- fileRange[1]
		outPar$max_intensity <- fileRange[5]
		outPar$zero_offset <- fileRange[3] 
		outPar$zero_offset <- 0
	}
	
	## Update sweep width and trasmitter frequencies
	if( !is.null(inFolder$w1) && !is.null(inFolder$w2) ){
		##SETH THIS SECTION IS NOT FINISHED
		outPar$spectrum_width_Hz 
		outPar$transmitter_MHz
	}
	
	## Update specific parameters if provided
	if( !missing(file.name) )
		outPar$file.name <- file.name
	if( !missing(file.size) )
		outPar$file.size <- file.size
	if( !missing(date.modified) )
		outPar$date.modified <- date.modified
	if( !missing(axis) )
		outPar$axis <- axis
	if( !missing(nucleus) )
		outPar$nucleus <- nucleus
	if( !missing(matrix_size) )
		outPar$matrix_size <- matrix_size
	if( !missing(block_size) )
		outPar$block_size <- block_size
	if( !missing(upfield_ppm) )
		outPar$upfield_ppm <- upfield_ppm
	if( !missing(downfield_ppm) )
		outPar$downfield_ppm <- downfield_ppm
	if( !missing(spectrum_width_Hz) )
		outPar$spectrum_width_Hz <- spectrum_width_Hz
	if( !missing(transmitter_MHz) )
		outPar$transmitter_MHz <- transmitter_MHz
	if( !missing(binary_location) )
		outPar$binary_location <- binary_location
	if( !missing(number_dimensions) )
		outPar$number_dimensions <- number_dimensions
	if( !missing(noise_est) )
		outPar$noise_est <- noise_est
	if( !missing(min_intensity) )
		outPar$min_intensity <- min_intensity
	if( !missing(max_intensity) )
		outPar$max_intensity <- max_intensity
	if( !missing(zero_offset) )
		outPar$zero_offset <- zero_offset
	if( !missing(user_title) )
		outPar$user_title <- user_title
	
	return(outPar)
}

## Limit the current spectrum to the data found within ROIs
## inFolder - list; data and file parameters for input spectra.  This should be 
##	similar to the output format of ucsf2D().  Defaults to the current spectrum.
## outPath - character string; full file path for output spectrum
## rois - data.frame; an ROI table, defaults to the current ROI table
## returnType - character string; specifies the function's return value format.
##	"path" indicates that the function should write the resulting spectrum out
##	to file and return the output path.  "folder" indicates that the resulting
##	spectrum should be returned as a list object similar to the inFolder
##	argument.
## inPath - character string; full file path for the input spectrum.  This is an 
##	alternative to the inFolder argument.
psuedoRsd <- function(inFolder, outPath, rois=roiTable, returnType='path', 
		inPath=NULL){
	
	## Default to the current spectrum
	if (missing(inFolder)){
		if (is.null(inPath))
			inFolder <- fileFolder[[wc()]]
	}else{
		
		## Check inFolder for the correct format
		if (is.null(inFolder$file.par))
			inFolder <- inFolder[[1]]
	}
	...
	## Get data for spectrum
	if (!is.null(inPath))
		inFolder <- ucsf2D(inPath)
	fp <- inFolder$file.par
	if (is.null(inFolder$data))
		inFolder <- ucsf2D(fp$file.name, file.par=fp)
	gc(FALSE)
	
	## Create a matrix of zeroes the same size as the spectrum
	if (fp$number_dimensions == 1)
		coeffMatrix <- rep(0, fp$matrix_size)
	else
		coeffMatrix <- matrix(0, fp$matrix_size[2], fp$matrix_size[1])
	
	## Replace the zeroes with ones in the areas of the matrix that contain an ROI
	for (i in 1:nrow(rois)) {
		
		if (fp$number_dimensions == 1){
			bounds <- matchShift(inFolder, w2=c(rois[i, 'w2_upfield'], 
							rois[i, 'w2_downfield']), return.inc=TRUE, invert=TRUE)
			coeffMatrix[bounds$w2[1]:bounds$w2[2]] <- 1
		}else{
			bounds <- matchShift(inFolder, w1=c(rois[i, 'w1_upfield'], 
							rois[i, 'w1_downfield']), w2=c(rois[i, 'w2_upfield'], 
							rois[i, 'w2_downfield']), return.inc=TRUE, invert=TRUE)
			coeffMatrix[bounds$w2[1]:bounds$w2[2], bounds$w1[1]:bounds$w1[2]] <- 1
		}
	}
	
	## Create the psuedo RSD spectrum
	outData <- coeffMatrix * inFolder$data
	inFolder$data <- outData
	
	if (!is.null(fp$block_upfield_ppms)){
		fp['block_upfield_ppms'] <- NULL
		fp['block_downfield_ppms'] <- NULL
		fp$block_size <- fp$matrix_size
		inFolder$file.par <- fp
	}
	
	## Write spectrum out to file
	if (returnType != 'folder'){
		writeUcsf(outPath, inFolder=inFolder, writeNoise=TRUE, writeShifts=TRUE, 
				cor=TRUE)
		return(outPath)
	}else
		return(inFolder)
...
## Increase/decreases the resolution of one or more spectra by interpolation
## inFolder - list; data and file parameters for input spectra.  This should be 
##	similar to the output format of ucsf2D(), but nested one level deeper in
##	order to accommodate multiple spectra.  Defaults to the current spectrum.
## outPath - character string; full file path for output spectrum
## w2Int - numeric; desired interval (PPM) between chemical shifts in the 
##	direct dimension
## w1Int - numeric; desired interval (PPM) between chemical shifts in the 
##	indirect dimension
## auto - logical; if TRUE, the chemical shift interval in each dimension will 
##	be calculated to produce a spectrum with a resolution capable of being
##	reduced by smoothDat() to match default hash file resolution.  If w2int or
##	w1Int are provided, this argument is set to FALSE.
## writeNoise - logical; if TRUE, the noise estimate for the original spectrum 
##	will be included in the header or the file.par for the output file.
## returnType - character string; specifies the function's return value format.
##	"path" indicates that the function should write the resulting spectrum out
##	to file and return the output path.  "folder" indicates that the resulting
##	spectrum should be returned as a list object similar to the inFolder
##	argument.
## inPath - character string; full file path for the spectra being interpolated.
##	This is an alternative to the inFolder argument.
interpSpec <- function(inFolder, outPath, w2Int=NULL, w1Int=NULL, auto=TRUE,
		writeNoise=FALSE, returnType='path', inPath=NULL){
	
	## Default to the current spectrum
	if (missing(inFolder) && is.null(inPath))
		inFolder <- fileFolder[wc()]
	
	## Check inFolder for the correct format
	if (is.null(inFolder[[1]]$file.par))
		inFolder <- list(inFolder)
	
	## Prompt user for output path
	if (returnType == 'path' && missing(outPath) && (length(inFolder) == 1 || 
				(!is.null(inPath) && length(inPath) == 1))){
		outPath <- mySave(defaultextension='.ucsf', title='Save spectrum', 
				filetypes=list('ucsf'='UCSF files'))
		if (!length(outPath) || !nzchar(outPath))
			return(invisible())
	}else if (!missing(outPath)) {
		if (missing(inFolder) && length(outPath) != length(inPath))
			err(paste('Output path vector must be the same length as the input', 
							'path vector'))
		if (!missing(inFolder) && length(outPath) != length(inFolder))
			err(paste('Output path vector must be the same length as the input', 
							'path vector'))
	}
	
	## Set auto to FALSE if intervals were specified explicitly
	if (!is.null(w2Int) && !is.null(w1Int))
		auto <- FALSE
	
	## Interpolate spectra one at a time
	if (is.null(inPath))
		inPath <- sapply(inFolder, function(x) x$file.par$file.name)
	n <- length(inPath)
	if (n > 1)
		cat('\nInterpolating spectra . . .', sep='')
	if (returnType == 'folder')
		outFolder <- NULL
	else
		outPaths <- NULL
	for (i in seq_along(inPath)){
		...
		if (missing(inFolder))
			prevSpec <- ucsf2D(inPath[i])
		else{
			if (is.null(inFolder[[i]]$data)){
				prevSpec <- ucsf2D(inPath[i], file.par=inFolder[[i]]$file.par)
			}else
				prevSpec <- inFolder[[i]]
		}
		fp <- prevSpec$file.par
		if (is.null(prevSpec$graphics.par))
			gp <- defaultSettings
		else
			gp <- prevSpec$graphics.par
		prevW1 <- prevSpec$w1
		prevW2 <- prevSpec$w2
		prevData <- prevSpec$data
		rm(prevSpec)
		
		## If intervals are not provided, calculate intervals for hash file creation
		defInt <- c(.1, .01)
		if (auto){
			currInt <- (fp$downfield_ppm - fp$upfield_ppm ) / (fp$matrix_size - 1)
			if (currInt[1] > defInt[1])
				w1Int <- defInt[1]
			else
				w1Int <- defInt[1] / 2^(ceiling(log2(defInt[1] / currInt[1])))
			if (currInt[2] > defInt[2])
				w2Int <- defInt[2]
			else
				w2Int <- defInt[2] / 2^(ceiling(log2(defInt[2] / currInt[2])))
		}
		
		## Determine where to interpolate points in the direct dimension
		w2 <- seq(-5, 15, w2Int)
		w2Range <- c(prevW2[1], prevW2[length(prevW2)])
		w2Range <- c(w2[which.min(abs(w2Range[1] - w2))], 
				w2[which.min(abs(w2Range[2] - w2))]) + c(-w2Int, w2Int)
		newW2 <- seq(w2Range[1], w2Range[2], w2Int)
		
		## Interpolate data along the direct dimension
		if (length(inPath) > 1){
			newData <- tryCatch(apply(prevData, 2, function(slice) 
								approx(prevW2, slice, newW2)$y), error=function(er){
						gc(FALSE)
						cat('Encountered error interpolating "', inPath[i], 
								'":\n ', er$message, 'Skipping.', sep='')
						flush.console()
						next
					})
		}else
			newData <- apply(prevData, 2, function(slice) 
						approx(prevW2, slice, newW2)$y)
		newData[which(is.na(newData))] <- 0
		rm(prevData)
		gc(FALSE)
		
		## Determine where to interpolate points in the indirect dimension
		w1 <- seq(-20, 220, w1Int)
		w1Range <- c(prevW1[1], prevW1[length(prevW1)])
		w1Range <- c(w1[which.min(abs(w1Range[1] - w1))], 
				w1[which.min(abs(w1Range[2] - w1))]) + c(-w1Int, w1Int)
		newW1 <- seq(w1Range[1], w1Range[2], w1Int)
		
		## Interpolate data along the indirect dimension
		if (length(inPath) > 1){
			outData <- tryCatch(apply(newData, 1, function(slice) 
								approx(prevW1, slice, newW1)$y), error=function(er){
						gc(FALSE)
						cat('Encountered error interpolating "', inPath[i], 
								'":\n ', er$message, 'Skipping.', sep='')
						flush.console()
			...
			outData <- apply(newData, 1, function(slice) 
						approx(prevW1, slice, newW1)$y)
		outData[which(is.na(outData))] <- 0
		rm(newData)
		gc(FALSE)
		
		## Set output name
		if (missing(outPath)){
			interpDir <- dirname(inPath[i])
			if (n > 1)
				interpDir <- file.path(interpDir, 'Interpolated Spectra')
			outName <- paste(interpDir, '/', 'interp_', basename(inPath[i]), sep='')
		}else
			outName <- outPath[i]
		
		## Setup output folder
		newFolder <- list()
		newFolder$file.par <- fp
		newFolder$w1 <- newW1
		newFolder$w2 <- newW2
		newFolder$data <- t(outData)
		if (returnType == 'folder'){
			outName <- basename(outName)
			newFolder$file.par <- parMod(newFolder, file.name=outName, 
					user_title=outName)
			if (writeNoise)
				newFolder$file.par$noise_est <- fp$noise_est
			newFolder$graphics.par <- gp
			outFolder[[i]] <- newFolder
			names(outFolder)[i] <- outName
		}else{
			
			## Write interpolated spectrum out to file
			newFolder$file.par <- parMod(newFolder)	
			writeUcsf(outName, inFolder=newFolder, writeShifts=TRUE, cor=TRUE, 
					writeNoise=writeNoise)
			outPaths <- c(outPaths, outName)
		}
	}
	if (n > 1)
		cat('\ndone\n\n')
...
## Lower the resolution of a spectrum by performing successive smoothings
## inFolder - list; data and file parameters for input spectrum.  This should 
##	match the	output format of ucsf2D().  Defaults to the current spectrum.
## outPath - character string; full file path to save the output spectra to
## w2Rep - numeric; number of successive smoothings to perform on the spectrum 
##	in the direct dimension.  Each time the data is smoothed, the number of
##	points is halved.
## w1Rep - numeric; number of successive smoothings to perform on the spectrum 
##	in the indirect dimension.  Each time the data is smoothed, the number of
##	points is halved.
## internal - logical; TRUE suppresses console output
## returnType - character string; specifies the function's return value format.
##	"path" indicates that the function should write the resulting spectrum out
##	to file and return the output path.  "folder" indicates that the resulting
##	spectrum should be returned as a list object similar to the inFolder
##	argument.
## inPath - character string; full file path for the spectrum being smoothed.
##	This is an alternative to the inFolder argument
smoothDat <- function( inFolder, outPath, w2Rep=NULL, w1Rep=NULL,	
		internal=FALSE, returnType='path', inPath=NULL){
	...
	if (fp$number_dimensions == 1){
		...
		for (i in 1:w2Rep){
			nR <- length(inFolder$data)
			
			## Set output sampling index
			idx <- (1:(ceiling(nR / 2))) * 2		
			inFolder$w2 <- inFolder$w2[idx - 1] ## Reverse indexing is for plotting							
			
			## Smooth and sample data, end points wrapped
			inFolder$data <- ((
					inFolder$data[c(nR, 1:(nR - 1))] * c(-1, rep(1, nR - 1)) + 
					inFolder$data + 
					inFolder$data[c(2:nR, 1)] * c(rep(1, nR - 1), -1)) / 3)[idx]
			gc(FALSE)
		}
	}else{
		
		## If the number of smoothings is not provided, smooth the spectrum to match
		##	hash file resolution, intervals of .01 (w2) and .1 (w1)
		if (is.null(w1Rep)){
			...
			## Caluculate number of succesive smoothings necessary
			w1Rep <- round(log2(defInt[1] / currInt[1]))
			w2Rep <- round(log2(defInt[2] / currInt[2]))
		}
		
		## Smooth 2D data
		for (i in 1:max(c(w2Rep, w1Rep))){
			
			## Smooth and sample direct data, end points wrapped
			if (i <= w2Rep){
				nR <- nrow(inFolder$data)
				idx <- (1:(ceiling(nR / 2))) * 2
				inFolder$data <- ((
								inFolder$data[c(nR, 1:(nR - 1)), ] * c(-1, rep(1, nR - 1)) + 
								inFolder$data + 
								inFolder$data[c(2:nR, 1), ] * c(rep(1, nR-1), -1)) / 3)	
				if (nR %% 2)
					inFolder$data <- rbind(rep(0, ncol(inFolder$data)), inFolder$data)
				inFolder$data <- inFolder$data[idx, ]
				inFolder$w2 <- inFolder$w2[idx - 1] ## Reverse indexing is for plotting	
				gc(FALSE)
			}
			
			## Smooth and sample indirect data, end points wrapped
			if (i <= w1Rep){	
				nC <- ncol(inFolder$data)	
				idx <- (1:(ceiling(nC / 2))) * 2
				inFolder$data <- ((t(t(inFolder$data[, c(nC, 1:(nC - 1))]) * 
												c(-1, rep(1, nC - 1))) + inFolder$data + 
								t(t(inFolder$data[,c(2:nC, 1)]) * c(rep(1, nC-1), -1))) / 3)
				if (nC %% 2)
					inFolder$data <- cbind(rep(0, nrow(inFolder$data)), inFolder$data)
				inFolder$data <- inFolder$data[, idx]
				inFolder$w1 <- inFolder$w1[idx - 1] ## Reverse indexing is for plotting
				gc(FALSE)
			}
		}			
	}
	...
	noiseEst <- fp$noise_est * .8^(w2Rep + w1Rep)
	inFolder$file.par <- parMod(inFolder, noise_est=noiseEst) 
	gc(FALSE)	
	
	## Write smoothed spectrum out to file
	if (returnType != 'folder'){
		writeUcsf(outPath, inFolder=inFolder, writeShifts=TRUE, cor=TRUE, 
				writeNoise=TRUE)
		return(outPath)
	}else
		return(inFolder)
...
## Smooth multiple spectra to match default hash resolution
## inDir - character string or vector; the input directory.  All ucsf files in
##	this directory will be processed
batchSmooth <- function(inDir, outDir){
	
	## Prompt user for input directory
	if (missing(inDir))
		inDir <- myDir()
	if (!length(inDir) || !nzchar(inDir))
		return(invisible())
	
	## Prompt user for output directory
	if (missing(outDir))
		outDir <- myDir()
	if (!length(outDir) || !nzchar(outDir))
		return(invisible())
	
	## Get paths to UCSF files within input directory
	ucsfFiles <- list.files(inDir, pattern=('sf|ucsf'), full.names=TRUE)
	
	## Smooth spectra one at a time
	outPaths <- NULL
	n <- length(ucsfFiles)
	for (i in seq_along(ucsfFiles)){
		
		## Get input and output paths
		inPath <- ucsfFiles[i]
		outPath <- file.path(outDir, paste('smoothed_', strsplit(basename(inPath), 
								'interp_')[[1]][2], sep=''))
		
		## Check if spectrum is already at correct resolution
		fp <- ucsfHead(inPath, print=FALSE)$file.par
		currInt <- (fp$downfield_ppm - fp$upfield_ppm ) / (fp$matrix_size - 1)
		defInt <- c(.1, .01)
		
		## Copy file to output folder if spectrum doesn't need to be smoothed
		if (all(abs(currInt - defInt) / defInt < .02)){
			cat('Copying spectrum:', i, 'of', n, '\n')
			file.copy(inPath, outPath)
			outPaths <- c(outPaths, outPath)
		}else{
			cat('Processing spectrum:', i, 'of', n, '\n')
			flush.console()
			
			## Smooth spectrum
			outPaths <- c(outPaths, smoothDat(inPath=inPath, outPath=outPath))
		}
	}
...
## Returns the chemical shift interval in each dimension for one or more spectra
## inPath - character string or vector; the full file paths for spectra
getShiftInt <- function(inPaths=currentSpectrum, inFolder=NULL){	
	
	##read headers
	if (is.null(inFolder)){
		for (i in seq_along(inPaths)){
			if (inPaths[i] %in% names(fileFolder))
				inFolder[[i]] <- fileFolder[[i]]
			else
				inFolder[[i]] <- ucsfHead(inPaths[i], FALSE)
		}
	}
	
	## Check inFolder for correct format
	if (is.null(inFolder[[1]]$file.par))
		inFolder <- list(inFolder)
	
	##calculate shift intervals
	intervals <- sapply(inFolder, function(x){
				(x$file.par$downfield_ppm - x$file.par$upfield_ppm) / 
						(x$file.par$matrix_size - 1)})
	
	##format output
	fileNames <- sapply(inFolder, function(x) x$file.par$file.name)
	intervals <- matrix(intervals, ncol=2, byrow=TRUE, 
			dimnames=list(fileNames, inFolder[[1]]$file.par$axis))
	
	return(intervals)
}

## Spectral math function, performs mathmatical operations on one or more 
##	spectra, returning a single, combined spectrum
## inFolder - list; data and file parameters for input spectra.  This should be 
##	similar to the output format of ucsf2D(), but nested one level deeper in
##	order to accommodate multiple spectra.  Defaults to the current spectrum.
## outPath - character string; full file path to save the output spectra to
## normFun - matrix-capable function or a text string ('s/n' or 'threshold')
##	functions will be applied to the input data matrix, 's/n' applies signal to 
##	noise normalization, 'threshold' sets all data over the observed noise level 
##	to 1, 'none' will skip the normalization step
## mathFun - string; Must match one of the following: 'add', 'subtract', 
##	'multiply', 'divide'. The operation is applied sequentially.
## internal - logical; TRUE suppresses console output
## returnType - character string; specifies the function's return value format.
##	"path" indicates that the function should write the resulting spectrum out
##	to file and return the output path.  "folder" indicates that the resulting
...
## inPath - character string; full file path for the spectra being interpolated.
##	This is an alternative to the inFolder argument.
mathNMR <- function( inFolder, outPath, normFun, mathFun = 'add', 
		internal=FALSE, returnType = 'path', inPath = NULL){
	
	## Create a list object for all spectra provided 
	if (missing(inFolder)){
		if (!is.null(inPath))
			inFolder <- NULL
		else
	...
	## Combine inFolder and inPath arguments into a single list object
	if (!is.null(inPath)){
		
		## Read in headers for inPath spectra
		inPathFolder <- NULL
		for (i in seq_along(inPath))
			inPathFolder[[i]] <- ucsfHead(inPath[i], FALSE)
		
		## Determine the order that the aruments were passed and combine folders
		if (!is.null(inFolder)){
			args <- names(sys.call())
			if (!'inFolder' %in% args || match('inFolder', args) < match('inPath', 
					args))
				inFolder <- c(inFolder, inPathFolder)
			else
				inFolder <- c(inPathFolder, inFolder)
		}else
			inFolder <- inPathFolder
		rm(inPathFolder)
		gc(FALSE)
		n <- length(inFolder)
	}
	
	## Find basic file information
	if (n > 1){
		nDim <- unique(sapply(inFolder, function(x) x$file.par$number_dimensions))
		if (length(nDim) > 1)
			stop('All files must have the same number of dimensions')
		w1Range <- range(sapply(inFolder, function(x) c(x$file.par$upfield_ppm[1], 
									x$file.par$downfield_ppm[1])))
		w2Range <-  range(sapply(inFolder, function(x) c(x$file.par$upfield_ppm[2], 
									x$file.par$downfield_ppm[2])))
		interval <- getShiftInt(inFolder=inFolder)
		meanInt <- apply(interval, 2, mean)
		intDelta <- cbind((interval[, 1] - meanInt[1]) / meanInt[1], 
				(interval[, 2] - meanInt[2]) / meanInt[2]) 
		if( any(abs(intDelta) > .02) )
			stop('All spectra must have the same resolution')
	}
	...
	## Check normFun argument
	if (missing(normFun))
		normFun <- as.list(rep('none', n))
	if (length(normFun) == 1 && !is.function(normFun))
		normFun <- as.list(rep(normFun, n))
	else if (length(normFun) != n)
		stop('normFun must be a list of length equal to the number of spectra')
	
	## Setup output matrix
	if (n > 1){
		w1Shifts <- seq( w1Range[1], w1Range[2], by = interval[1, 1] )
		w2Shifts <- seq( w2Range[1], w2Range[2], by = interval[1, 2] )
		np <- c(length(w1Shifts), length(w2Shifts))
		outDat <- matrix(0, nrow=np[2], ncol=np[1])
	}else
		w1Shifts <- w2Shifts <- np <- outDat <- NULL
	
	## Combine spectra using the function provided
	for (i in 1:n){
		...
		## Get data for the spectrum if not provided
		if (is.null(inFolder[[i]]$data)){
			inDat <- ucsf2D(inFolder[[i]]$file.par$file.name, 
					file.par=inFolder[[i]]$file.par)
		}else{
			inDat <- NULL
			inDat$w1 <- inFolder[[i]]$w1
			inDat$w2 <- inFolder[[i]]$w2
			inDat$data <- inFolder[[i]]$data
		}
		if (n == 1){
			w1Shifts <- inDat$w1
			w2Shifts <- inDat$w2 
		...
		if ( is.function(normFun[[i]]) ){
			inDat$data <- normFun[[i]]( inDat$data )	
		}else{
			
			## Signal to noise normilization
			if( normFun[[i]] == 's/n' )
				inDat$data <- inDat$data / inFolder[[i]]$file.par$noise_est  					
			
			## Digitized spectrum				
			if( normFun[[i]] == 'threshold'){
				if (is.null(inFolder[[i]]$graphics.par))
					gp <- defaultSettings
				else
					gp <- inFolder[[i]]$graphics.par
				conDisp <- gp$conDisp
				thresh <- inFolder[[i]]$file.par$noise_est * gp$clevel
				if( all(conDisp) ){
					inDat$data <- abs(inDat$data)		
				}else{
					if( conDisp[2] )
						inDat$data <- inDat$data * -1
				}
				
				## Convert data to obs versus not obs matrix
				inDat$data <- matrix( inDat$data > thresh, nrow = length(inDat$w2), 
						ncol = length(inDat$w1)) 
			}
			gc(FALSE)
		}
		
		## Find the correct start/end index
		w1End <- which(rev(w1Shifts) <= inDat$w1[1])[1]
		w1Start <- w1End - length(inDat$w1) + 1
		w2End <- which(rev(w2Shifts) <= inDat$w2[1])[1]
		w2Start <- w2End - length(inDat$w2) + 1
		
		## Apply the function and create a new spectrum
		if( i == 1 || mathFun == 'add' ){
			outDat[w2Start:w2End, w1Start:w1End] <- 
					outDat[w2Start:w2End, w1Start:w1End] + inDat$data			
		}else if( mathFun == 'subtract' ){
			outDat[w2Start:w2End, w1Start:w1End] <- 
					outDat[w2Start:w2End, w1Start:w1End] - inDat$data	
		}else if( mathFun == 'multiply' ){
			outDat[w2Start:w2End, w1Start:w1End] <- 
					outDat[w2Start:w2End, w1Start:w1End] * inDat$data	
		}else if( mathFun == 'divide' ){
			outDat[w2Start:w2End, w1Start:w1End] <- 
					outDat[w2Start:w2End, w1Start:w1End] / inDat$data	
		}
		rm(inDat)
	...
	## Setup output folder
	outFolder <- NULL
	outFolder$file.par <- inFolder[[1]]$file.par
	outFolder$w1 <- w1Shifts
	outFolder$w2 <- w2Shifts
	outFolder$data <- outDat
	outFolder$file.par <- parMod(outFolder)
	outFolder$graphics.par <- inFolder[[1]]$graphics.par
	if ('threshold' %in% normFun){
		outFolder$file.par$noise_est <- .1
		writeNoise <- TRUE
	}else
		writeNoise <- FALSE
	rm(outDat)
	...
	## Write out the final matrix as a UCSF file
	if (returnType != 'folder'){
		writeUcsf(outPath, inFolder=outFolder, writeShifts=TRUE, cor=TRUE, 
				writeNoise=writeNoise)
		return(outPath)
	}else
		return(outFolder)
...
## Create a probability matrix for the current spectrum
probMat <- function(){
	
	## Get data for current spectrum
	current <- wc()
	inFolder <- ucsf2D(currentSpectrum, file.par=fileFolder[[current]]$file.par)
	
	## Get hash data
	hashPaths <- paste(defaultSettings$searchLibs, 'hash.ucsf', sep='/')
	if (length(hashPaths) > 1)
		libHash <- mathNMR(inPath=hashPaths, internal=TRUE, returnType='folder')
	else
		libHash <- ucsf2D(hashPaths)
	
	## Interpolate and smooth current spectrum to match hash resolution
	highRes <- interpSpec(returnType='folder')
	matchRes <- smoothDat(highRes, returnType='folder', internal=TRUE)
	rm(highRes)
	gc(FALSE)
	
	## Create probability matrix
	libHash$data <- 1 / libHash$data
	libHash$data[which(is.infinite(libHash$data))] <- 0
	mathFolder <- list(matchRes, libHash)
	rm(list=c('libHash', 'matchRes'))
	gc(FALSE)
	probMatrix <- mathNMR(mathFolder, normFun=list('threshold', 'none'), 
			mathFun='multiply', internal=TRUE, returnType='folder')
	
	## Save matrix to global environment
	assignments$probMatrix <- probMatrix
	myAssign('assignments', assignments, FALSE)
}

## Assignment function
autoAssign <- function(w_numRes = 0.5, t_numRes = 10){
	
	## Basic setup
	current <- wc()
	if(!is.null(assignments) && (any(assignments$table$Score == 1) || 
				any(assignments$table$Score == 0)))
		checkPrev <- TRUE
	else
		checkPrev <- FALSE
	
	## Get combined ROI record from provided library locations
	if (is.null(assignments)){
		roiRecord <- read.table(file.path(defaultSettings$searchLibs[1], 
						'roiRecord.roi'), head=TRUE, sep='\t', stringsAsFactors=FALSE)
		roiRecord$Library <- rep(defaultSettings$searchLibs[1], nrow(roiRecord))
		for (i in defaultSettings$searchLibs[-1]){
			currRec <- read.table(file.path(i, 'roiRecord.roi'), head=TRUE, sep='\t', 
					stringsAsFactors=FALSE)
			currRec$Library <- rep(i, nrow(currRec))
			roiRecord <- merge(roiRecord, currRec, all=TRUE)
		}
	}else
		roiRecord <- assignments$table
	
#	if( is.null(roiRec$Library))
#		roiRec$Library <- rep(NA, nrow(roiRec))
	
	## Remove this code when 1D/TOCSY is supported
#	if( fileFolder[[current]]$file.par$number_dimensions != 2 )
#		stop('Currently autoAssign is for 2D HSQC only')
#	if( !all(fileFolder[[current]]$file.par$nucleus == c("13C", "1H")) )
#		stop('Currently autoAssign is for 2D HSQC only')
	
	## Set matching threshold
	thresh <- fileFolder[[current]]$file.par$noise_est * 
			fileFolder[[current]]$graphics.par$clevel
	
	## Look for data over noise threshold in each ROI
	if (checkPrev)
		datObs <- roiRecord$Obs_Signal
	else{
		datObs <- rep(FALSE, nrow(roiRecord) )
		for( i in 1:nrow(roiRecord)){
			w1Range <- as.numeric(roiRecord[i, 4:5])
			w2Range <- as.numeric(roiRecord[i, 2:3])
			roiDat <- ucsf2D(file.name = wc(TRUE), w1Range = w1Range, 
					w2Range = w2Range, file.par=fileFolder[[current]]$file.par )
			datObs[i] <- length(which( roiDat$data > thresh)) > 6	
		}
	}
	
	## Parse the roiRecord
	roiRecord$ACTIVE <- FALSE
	if (any(datObs))
		roiRecord[which(datObs == 1),]$ACTIVE <- TRUE
	cNames <- sapply(strsplit(roiRecord$Name, ".", fixed = TRUE), function(x) 
				x[1])
	uNames <- unique(cNames)
	rMatch <- rTot <- NULL
	
	## Create base score from obs versus expected number of ROIs
	for(i in 1:nrow(roiRecord)){
		currRois <- which(cNames == cNames[i])
		rMatch <- c(rMatch, length(which(roiRecord[currRois, 'ACTIVE'])))
		rTot <- c(rTot, length(currRois))
	}
	subScore <- 1-((rTot - rMatch) / rTot *.90 + .05)
	
	## Calculate standard's complexity probability 
	## This is used to penalize standards with few ROIs
	rTot[ rTot > t_numRes ] <- t_numRes
	p_numRes <- (1- (rTot/t_numRes - 1/t_numRes)/(1 - 1/t_numRes) ) 
	
	## Calculate transition probability for peak number mismatch
	## This is used to penalize mismatch between obs vs epected transitions
#	peakList <- peakPick( wc(TRUE), w1Range = w1Range, w2Range = w2Range, 
#			internal = TRUE)
	
	
	## Calculate total match probablity
	subScore <- subScore - p_numRes * w_numRes
	subScore[ subScore < .05 ] <- .05
	
	## Use data from previous assignments
	if( checkPrev ){
		acc <- which(roiRecord$Score == 1)
		if( length(acc) )
			subScore[acc] <- 1
		rej <- which(roiRecord$Score == 0)
		if( length(rej) )
			subScore[rej] <- 0
	}
	
	## Create assignment table and assign to the global environment
	roiRecord$Score <- subScore
	roiRecord$Obs_Signal <- datObs
	if (!checkPrev)
		roiRecord <- orderROI(roiRecord)
	assignments$table <- roiRecord
	myAssign('assignments', assignments)
}

## Automatic assignment GUI
aa <- function(){
	...
	if (is.null(dlg))
		return(invisible())
	tkwm.title(dlg, 'Auto Assign')
	tkwm.resizable(dlg)
	tkfocus(dlg)
	tkwm.deiconify(dlg)
	localAssign <- NULL
	
	##create library tablelist widget
	libFrame <- ttklabelframe(dlg, text='Available libraries')
	xscrLib <- ttkscrollbar(libFrame, orient='horizontal', command=function(...) 
				tkxview(libTable, ...))
	yscrLib <- ttkscrollbar(libFrame, orient='vertical', command=function(...) 
				tkyview(libTable, ...))
	libTable <- tkwidget(libFrame, 'tablelist::tablelist', bg='white',  
			columns=c('20', 'Library', '8', 'Direct nuclues', 'center', '8', 
					'Indirect nuclues', 'center', '5', 'Search', 'center'), 
			height=3,	width=80, spacing=3, stretch='all', activestyle='underline', 
			exportselection=FALSE, selectmode='single',
			editstartcommand=function(...) preSearchEdit(...),
			editendcommand=function(...) postSearchEdit(...),
			xscrollcommand=function(...) tkset(xscrLib, ...),
			yscrollcommand=function(...) tkset(yscrLib, ...))
	
	##add library paths to table
	libNames <- 'BMRB HSQC library'
	defLibs <- createObj('defaultSettings', returnObj=TRUE)$libLocs
	for (i in defaultSettings$libLocs){
		libMatch <- match(i, defLibs)
		if (is.na(libMatch))
			libText <- i
		else
			libText <- libNames[libMatch]
		nucs <- unlist(dget(file.path(i, 'metadata.txt'))[c('Direct_nucleus', 
								'Indirect_nucleus')])
		rowVals <- c(libText, nucs, ifelse(i %in% defaultSettings$searchLibs, 'yes', 
						'no'))
		tkinsert(libTable, 'end', rowVals)
	}
	
	##configure search column to include combobox widgets column
	tcl(libTable, 'columnconfigure', 3, editable=TRUE, editwindow='ttk::combobox')
	preSearchEdit <- function(tbl, row, col, newVal){
		tkconfigure(tcl(libTable, 'entrypath'), state='readonly', values=c('yes', 
						'no'), exportselection=FALSE)
		return(tclVar(newVal))
	}
	
	##check edited value in search column
	postSearchEdit <- function(tbl, row, col, newVal){
		
		##ensure nuclei for selected libraries match
		if (as.character(newVal) == 'yes'){
			
			##get the nuclei for the selected library
			selLib <- defaultSettings$libLocs[as.numeric(row) + 1]
			metadata <- dget(file.path(selLib, 'metadata.txt'))
			libNuc <- as.vector(unlist(metadata[c('Indirect_nucleus', 
											'Direct_nucleus')]))
			
			##ensure the nuclei for the selected library match the current spectrum
			currNuc <- fileFolder[[currentSpectrum]]$file.par$nucleus
			if (!identical(currNuc, libNuc)){
				tcl(tbl, 'cancelediting')
				err(paste('The nuclei for the spectra in the selected library\ndo not',
								'match the nuclei for the current spectrum.\nPlease select a',
								'different library.'), dlg)
			}
		}
		return(tclVar(newVal))
	}
	
	##modify defaultSettings after editing search libraries
	onLibChange <- function(){
		selLibs <- which(as.character(tcl(libTable, 'getcolumns', 3)) == 'yes')
		searchLibs <-	defaultSettings$libLocs[selLibs]
		if (!identical(searchLibs, defaultSettings$searchLibs)){
			defaultSettings$searchLibs <- searchLibs
			myAssign('defaultSettings', defaultSettings)
			writeDef()
		}
	}
	tkbind(libTable, '<<TablelistCellUpdated>>', onLibChange)
	
	##create assignments tablelist widget
	assignFrame <- ttklabelframe(dlg, text='Assignment Table')
	xscrAssign <- ttkscrollbar(assignFrame, orient='horizontal', 
			command=function(...) tkxview(assignTable, ...))
	yscrAssign <- ttkscrollbar(assignFrame, orient='vertical', 
			command=function(...) tkyview(assignTable, ...))
	assignTable <- tkwidget(assignFrame, 'tablelist::tablelist', bg='white',  
			columns=c('24', 'Name', '8', 'Score', '9', 'Status', 'center'), 
			height=8, width=50, selectmode='extended', spacing=3, stretch='all', 
			activestyle='underline', exportselection=FALSE,
			labelcommand=function(...) onAssignSort(...),
			xscrollcommand=function(...) tkset(xscrAssign, ...),
			yscrollcommand=function(...) tkset(yscrAssign, ...))
	
	##format text in Status column
	...
	tcl(assignTable, 'columnconfigure', 2, formatcommand=function(...) 
				formatStatus(...))
	...
	dispStatus <- function(){
		for (i in 0:(nrow(localAssign) - 1)){
			if (localAssign$Status[i + 1] == 'accepted')
				tcl(assignTable, 'cellconfigure', paste(i, 2, sep=','), 
						image='checkmark')
			else if (localAssign$Status[i + 1] == 'rejected')
				tcl(assignTable, 'cellconfigure', paste(i, 2, sep=','), image='redx')
		}
	}
	
	##sorts columns in tablelist
	onAssignSort <- function(tbl, col, order='default'){
		
		##sort tablelist by columns
		prevOrder <- as.character(tcl(assignTable, 'getcolumns', 0))
		if (order == 'default'){
			prevSort <- tclvalue(tcl(assignTable, 'sortorder'))
			if (prevSort == 'decreasing')
				tcl(assignTable, 'sortbycolumn', col, '-increasing')
			else
				tcl(assignTable, 'sortbycolumn', col, '-decreasing')
		}else
			tcl(assignTable, 'sortbycolumn', col, paste('-', order, sep=''))
		newOrder <- as.character(tcl(assignTable, 'getcolumns', 0))
		newOrder <- match(newOrder, prevOrder)
		localAssign <<- localAssign[newOrder, ]
		
		##apply new sorting order to assignment table
		newOrder <- NULL
		for (i in localAssign$Comp_ID)
			newOrder <- c(newOrder, grep(i, assignments$table$Comp_ID))
		assignments$table <- assignments$table[newOrder, ]
		myAssign('assignments', assignments)
		dispStatus()
	}
	
	##updates tablelist in GUI to match assignment table
	updateLocal <- function(){
		
		##format assignment table for display in GUI
		if (is.null(assignments)){
			localAssign <<- NULL
			tkdelete(assignTable, 0, 'end')
			return(invisible())
		}
		uniqRows <- match(unique(assignments$table$Comp_ID), 
				assignments$table$Comp_ID)
		guiCols <- match(c('Name', 'Score', 'Score', 'Comp_ID'), 
				names(assignments$table))
		localAssign <- assignments$table[uniqRows, guiCols]
		colnames(localAssign)[3] <- 'Status'
		localAssign$Status[which(localAssign$Status != 0 & 
								localAssign$Status != 1)] <- 'unverified'
		localAssign$Status[which(localAssign$Status == 1)] <- 'accepted'
		localAssign$Status[which(localAssign$Status == 0)] <- 'rejected'
		localAssign$Name <- sapply(strsplit(localAssign$Name, '.', fixed=TRUE), 
				function(x) x[1])
		localAssign <<- localAssign
		
		##update tablelist
		usrSel <- as.numeric(tcl(assignTable, 'curselection'))
		tkdelete(assignTable, 0, 'end')
		for (i in 1:nrow(localAssign))
			tkinsert(assignTable, 'end', unlist(localAssign[i, c(1:3)]))
		tkselection.set(assignTable, usrSel)
		dispStatus()
	}
	updateLocal()
	...
	##selects all rows when Ctrl+A is pressed
	tkbind(dlg, '<Control-a>', function(...) 
				tkselection.set(assignTable, 0, 'end'))
	
	##create verify selected button
	bottomFrame <- ttkframe(assignFrame)
	onVerSel <- function(){
		
		##get user selection
		if (is.null(localAssign))
			err('You must generate an assignment table before verifying assignments.', 
					dlg)
		usrSel <- as.numeric(tcl(assignTable, 'curselection'))
		if (!length(usrSel))
			err('You must select an assignment from the table to verify.', dlg)
		selIds <- localAssign$Comp_ID[usrSel + 1]
		
		##display verification GUI
		verGui(dlg, selIds)
	}
	verSelButton <- ttkbutton(bottomFrame, text='Verify Selected . . .', width=20, 
			command=onVerSel)
	
	##create remove assignments button
	onRemAssign <- function(){
		...
		usrSel <- as.numeric(tcl(assignTable, 'curselection'))
		if (!length(usrSel))
			err('You must select an assignment from the table to remove.', dlg)
		selIds <- localAssign$Comp_ID[usrSel + 1]
		
		##remove selected assignments and update
		remIndices <- NULL
		for (i in selIds)
			remIndices <- c(remIndices, grep(i, assignments$table$Comp_ID))
		assignments$table <- assignments$table[-remIndices, ]
		if (!nrow(assignments$table))
			assignments <- NULL
		myAssign('assignments', assignments)
		updateLocal()
	}
	remAssignButton <- ttkbutton(bottomFrame, text='Remove Selected', width=22, 
			command=onRemAssign)
	
	##create generate assignment list button
	rightFrame <- ttkframe(dlg)
	onGenerate <- function(){
		if (!as.numeric(tcl(libTable, 'finishediting')))
			return(invisible())
		if (!length(defaultSettings$searchLibs))
			err(paste('You must select at least one library for use in assignment', 
							'searches.'), dlg)
		tkconfigure(dlg, cursor='watch')
		myAssign('assignments', NULL, FALSE)
		autoAssign()
		updateLocal()
		onAssignSort(assignTable, 1, order='decreasing')
		tkconfigure(dlg, cursor='arrow')
	}
	genButton <- ttkbutton(rightFrame, text='Generate Assignments', width=20, 
			command=onGenerate)
	
	##create verify assignments button
	onVer <- function(){
		if (is.null(localAssign))
			err('You must generate an assignment table before verifying assignments.',
					dlg)
	...
	verButton <- ttkbutton(rightFrame, text='Verify Assignments . . .', width=20, 
			command=onVer)
	
	##create add assignments button
	onAddAssign<- function(){
		
		##check for an assignment table
		if (is.null(localAssign))
			err('You must generate an assignment table before adding assignments.',
					dlg)
		
		##select a library
		tcl(libTable, 'finishediting')
		selLibs <- as.logical(tcl(libTable, 'getcolumns', 3))
		if (!any(selLibs))
			err(paste('You must select at least one library for use in assignment', 
							'searches.'), dlg)
		libNames <- as.character(tcl(libTable, 'getcolumns', 0))[selLibs]
		if (length(libNames) > 1)
			usrSel <- mySelect(libNames, title='Select a library:', parent=dlg, 
					index=TRUE)
		else
			usrSel <- 1
		if (!nzchar(usrSel))
			return(invisible())
		usrLib <- defaultSettings$searchLibs[usrSel]
		
		##read the ROI record for the selected library
		roiRecord <- read.table(file.path(usrLib, 'roiRecord.roi'), head=TRUE, 
				sep='\t', stringsAsFactors=FALSE)
		
		##display selection window with all available compounds from the library
		compNames <- sapply(strsplit(roiRecord$Name, '.', fixed=TRUE), function(x) 
					x[1])
		fullNames <- unique(paste(compNames, ' (', roiRecord$Comp_ID, ')', 
						sep=''))
		ids <- sapply(strsplit(fullNames, ' (', fixed=TRUE), function(x) x[2])
		ids <- sapply(strsplit(ids, ')', fixed=TRUE), function(x) x[1])
		assignMatches <- unique(match(assignments$table$Comp_ID, ids, nomatch=0))
		if (any(assignMatches > 0))
			fullNames <- fullNames[-assignMatches]
		if (!length(fullNames))
			err(paste('All standards for the selected library are already included', 
							'in the assignment table.'), dlg)
		usrSel <- mySelect(fullNames, multi=TRUE, title='Select standard(s):', 
				parent=dlg)
		if (!nzchar(usrSel[1]))
			return(invisible())
		
		##add compounds to assignment table
		ids <- NULL
		for (i in usrSel){
			id <- strsplit(i, ' (', fixed=TRUE)[[1]][2]
			id <- strsplit(id, ')', fixed=TRUE)[[1]][1]
			ids <- c(id, ids)
			currAssign <- roiRecord[which(roiRecord$Comp_ID == id), ]
			currAssign$Library <- usrLib
			currAssign$Score <- .99
			
			##check for observed signal within each ROI
			thresh <- fileFolder[[currentSpectrum]]$file.par$noise_est * 
					fileFolder[[currentSpectrum]]$graphics.par$clevel
			datObs <- rep(FALSE, nrow(currAssign))
			for (i in 1:nrow(currAssign)){
				w1Range <- as.numeric(currAssign[i, 4:5])
				w2Range <- as.numeric(currAssign[i, 2:3])
				roiDat <- ucsf2D(currentSpectrum, w1Range=w1Range, w2Range=w2Range, 
						file.par=fileFolder[[currentSpectrum]]$file.par)
				datObs[i] <- length(which(roiDat$data > thresh)) > 6	
			}
			currAssign$Obs_Signal <- datObs
			assignments$table <- rbind(assignments$table, currAssign)
		}
		myAssign('assignments', assignments)
		updateLocal()
		tkyview.moveto(assignTable, 1)
		tkselection.clear(assignTable, 0, 'end')
		tkselection.set(assignTable, match(ids, localAssign$Comp_ID) - 1)
	}
	addButton <- ttkbutton(rightFrame, text='Add Assignment', width=20, 
			command=onAddAssign)
	
	##create save assignments button
	onSave <- function(){
		
		##check for existing assignments
		if (is.null(assignments))
			err('You must generate assignments before saving.', dlg)
		
		##get ROIs for accepted assignments
		newRois <- assignments$table[which(assignments$table$Score == 1), ]
		
		##check with user for overwrite if ROI table already exists
		overAppend <- 'Overwrite'
		if (exists('roiTable') && length(roiTable) > 0)
			overAppend <- buttonDlg('An ROI table already exists.', c('Overwrite', 
							'Append', 'Cancel'), default='Cancel', parent=dlg)
		if (overAppend == 'Cancel')
			return(invisible())
		...
		if (overAppend == 'Append'){
			...
			##check for matching columns in the two tables
			newNames <- colnames(newRois)
			oldNames <- colnames(roiTable)
			if (!identical(newNames, oldNames)){
				...
				##add any columns from the existing table missing from the new table
				for (i in oldNames[-match(newNames, oldNames, nomatch=0)]){
					newRois <- cbind(newRois, rep(NA, nrow(newRois)))
					colnames(newRois)[ncol(newRois)] <- i
				}
			}
			newRois <- rbind(roiTable, newRois)
		}
		
		## Assigns the ROI table to the global environment
		myAssign('roiTable', newRois)		
		refresh()
	}
	saveButton <- ttkbutton(rightFrame, text='Save Assignments', width=20, 
			command=onSave)
	
	##ensure nuclei for search libraries match the nuclei for the current spectrum
	onMouse <- function(){
		
		##get search libraries
		searchLibs <- defaultSettings$searchLibs
		if (!length(searchLibs))
			return(invisible())
		
		##get nuclei for the current spectrum
		currNuc <- fileFolder[[currentSpectrum]]$file.par$nucleus
		
		##check libraries one at time
		for (i in seq_along(searchLibs)){
			
			##get nuclei for library
			metadata <- dget(file.path(searchLibs[i], 'metadata.txt'))
			libNuc <- as.vector(unlist(metadata[c('Indirect_nucleus', 
											'Direct_nucleus')]))
			
			##remove library from search libraries if nuclei do not match
			if (!identical(currNuc, libNuc)){
				tcl(libTable, 'cellconfigure', paste(i - 1, 3, sep=','), text=0, 
						image='unchecked')
				onLibChange()
			}
		}
	...
	##add widgets to libFrame
	tkgrid(libFrame, column=1, columnspan=2, row=1, sticky='nswe', pady=6, 
			padx=c(8, 0))
	tkgrid(libTable, column=1, row=1, sticky='nswe')
	tkgrid(xscrLib, column=1, row=2, sticky='we')
	tkgrid(yscrLib, column=2, row=1, sticky='ns')
	
	##make libFrame stretch when window is resized
	tkgrid.columnconfigure(dlg, 1, weight=1)
	tkgrid.rowconfigure(dlg, 1, weight=1)
	tkgrid.columnconfigure(libFrame, 1, weight=1)
	tkgrid.rowconfigure(libFrame, 1, weight=1)
	
	##add widgets to assignFrame
	tkgrid(assignFrame, column=1, row=2, sticky='nswe', pady=c(0, 6), padx=8)
	tkgrid(assignTable, column=1, row=1, sticky='nswe')
	tkgrid(xscrAssign, column=1, row=2, sticky='we')
	tkgrid(yscrAssign, column=2, row=1, sticky='ns')
	
	##add widgets to bottomFrame
	tkgrid(bottomFrame, column=1, columnspan=2, row=3, pady=4)
	tkgrid(verSelButton, column=1, row=1, pady=2, padx=c(0, 6))
	tkgrid(remAssignButton, column=2, row=1, pady=2)
	
	##make tableFrame stretch when window is resized
	tkgrid.rowconfigure(dlg, 2, weight=1)
	tkgrid.columnconfigure(assignFrame, 1, weight=1)
	tkgrid.rowconfigure(assignFrame, 1, weight=1)
	
	##add widgets to rightFrame
	tkgrid(rightFrame, column=2, row=2, sticky='nswe', pady=4)
	tkgrid(genButton, column=1, row=1, pady=c(45, 12))
	tkgrid(verButton, column=1, row=3)
	tkgrid(addButton, column=1, row=4, pady=12)
	tkgrid(saveButton, column=1, row=5)
	tkgrid(ttksizegrip(dlg), column=3, row=3, sticky='se')
	
	invisible()
}

## Assignment verification GUI
verGui <- function(parent, selIds){
	
	##save original plot settings
	prevOverlays <- overlayList
	prevGraphPar <- fileFolder[[currentSpectrum]]$graphics.par
	prevRoiTable <- roiTable
	prevGlobal <- globalSettings
	
	##create main window
	if (is.null(assignments))
		err('You must generate an assignment table before verifying assignments.')
	tt <- tktoplevel(parent=parent)
	tcl('wm', 'attributes', tt, topmost=TRUE)
	if (.Platform$OS == 'windows')
		tcl('wm', 'attributes', tt, toolwindow=TRUE)
	tkwm.title(tt, 'Verify Assignments')
	tkwm.resizable(tt)
	tkfocus(tt)
	tkwm.deiconify(tt)
	
	##limit ROI record to selected compounds
	if (missing(selIds)){
		roiRec <- assignments$table
	}else{
		matches <- NULL
		for (i in selIds)
			matches <- c(matches, grep(i, assignments$table$Comp_ID))
		roiRec <- assignments$table[matches, ]
	}
	
	##define local variables
	currId <- currRoi <- fileNames <- NULL
	libLocs <- unique(roiRec$Library)
	idList <- unique(roiRec$Comp_ID)
	
	##open files for verification
	allFiles <- list.files(libLocs, pattern='.rsd', full.names=TRUE)
	for (i in idList)
		fileNames <- c(fileNames, allFiles[grep(i, allFiles, fixed=TRUE)])
	if (!is.null(fileNames))
		silentOpen(fileNames)
	
	##updates the assignment table in the auto assignment GUI
	updateAA <- function(){
		
		##format assignment table for display in auto assignment GUI
		tableList <- '.aa.2.3'
		if (is.null(assignments)){
			tkdelete(tableList, 0, 'end')
			return(invisible())
		...
		##update auto assignment GUI text
		usrSel <- as.numeric(tcl(tableList, 'curselection'))
		tkdelete(tableList, 0, 'end')
		for (i in 1:nrow(localAssign))
			tkinsert(tableList, 'end', unlist(localAssign[i, 1:3]))
		tkselection.set(tableList, usrSel)
		
		##display status symbols in assignment table
		for (i in 0:(nrow(localAssign) - 1)){
			if (localAssign$Status[i + 1] == 'accepted')
				tcl(tableList, 'cellconfigure', paste(i, 2, sep=','), image='checkmark')
			else if (localAssign$Status[i + 1] == 'rejected')
				tcl(tableList, 'cellconfigure', paste(i, 2, sep=','), image='redx')
		}
	}
	
	##display an assignment spectrum and load ROI table
	getAssign <- function(id){
		
		##overlay assignment spectrum
		fileName <- fileNames[grep(id, fileNames, fixed=TRUE)]
		setGraphics(fileName, pos.color='red', neg.color='red', save.backup=FALSE)
		myAssign('overlayList', fileName, save.backup=FALSE)
		currId <<- id
		
		##configure plot colors to ensure overlays and ROIs can be seen
		if (prevGraphPar$bg == 'white' || prevGraphPar$bg == '#ffffff'){
			setGraphics(bg='white', line.color='black',	pos.color='blue', 
					neg.color='green', roi.bcolor=c('black', 'black'), 
					roi.tcolor=c('black', 'black'), roi.cex=c(.6, .6), 
					roi.lty=c('dashed', 'dashed'), roi.labelPos='left', save.backup=FALSE)
		}else{
			setGraphics(bg='black', line.color='white',	pos.color='blue', 
					neg.color='green', roi.bcolor=c('white', 'white'), 
					roi.tcolor=c('white', 'white'), roi.cex=c(.6, .6), 
					roi.lty=c('dashed', 'dashed'), roi.labelPos='left', save.backup=FALSE)
		}
		
		
		##get ROIs for assignment spectrum
		roiTable <- roiRec[grep(id, roiRec$Comp_ID), ]
		rownames(roiTable) <- NULL
		roiTable$ACTIVE <- rep(FALSE, nrow(roiTable))
		myAssign('roiTable', roiTable, save.backup=FALSE)
		currRoi <<- 'All'
		
		##set zoom level so assignment overlay is fully visible
		filePar <- fileFolder[[fileName]]$file.par
		usr <- c(filePar$downfield_ppm[2], filePar$upfield_ppm[2],
				filePar$downfield_ppm[1], filePar$upfield_ppm[1])
		p <- -1
		usr[1:2] <- rev(sort(newRange(usr[1:2], f = -p/100)))
		usr[3:4] <- rev(sort(newRange(usr[3:4], f = -p/100)))	
		setGraphics(usr=usr, save.backup=FALSE, refresh=TRUE)
		showRoi(roiTable)
		
		##configure ROI text
		tkconfigure(roiText, state='normal')
		tkdelete(roiText, '1.0', 'end')
		tkinsert(roiText, '1.0', 'Current ROI:\n', 'label')
		tkinsert(roiText, '2.0', currRoi, 'roiName')
		tcl(roiText, 'tag', 'configure', 'roiName', foreground='red')
		tkconfigure(roiText, state='disabled')
		
		##configure ROI buttons
		tkconfigure(nextRoiButton, state='normal', text='View')
		tkconfigure(prevRoiButton, state='disabled')
		
		##configure assignment text and auto assignment GUI
		updateAssign()
		updateAA()
		
		##configure assignment buttons
		i <- match(id, idList)
		if (i == length(idList))
			tkconfigure(nextAssignButton, state='disabled')
		else
			tkconfigure(nextAssignButton, state='normal')
		if (i == 1)
			tkconfigure(prevAssignButton, state='disabled')
		else
			tkconfigure(prevAssignButton, state='normal')
	}
	
	##create ROI description text
	roiFrame <- ttklabelframe(tt, text='ROI')
	roiText <- tktext(roiFrame, width=19, height=6, wrap='none', 
			font='TkDefaultFont', cursor='arrow', state='disabled', 
			xscrollcommand=function(...) tkset(roiXscr, ...))
	roiXscr <- ttkscrollbar(roiFrame, orient='horizontal',
			command=function(...) tkxview(roiText, ...))
	...
	updateRoi <- function(){
		...
		tkdelete(roiText, '1.0', 'end')
		tkinsert(roiText, '1.0', 'Current ROI:\n', 'label')
		tkinsert(roiText, '2.0', paste(currRoi, '\n', sep=''), 'roiName')
		tkinsert(roiText, '4.0', '\nAtoms:\n', 'label')
		currAtoms <- roiTable$Assignment[match(currRoi, roiTable$Name)]
		tkinsert(roiText, '5.0', currAtoms, 'atoms')
		tcl(roiText, 'tag', 'configure', 'roiName', foreground='red', 
				font='TkFixedFont')
		tcl(roiText, 'tag', 'configure', 'atoms', foreground='red', 
				font='TkFixedFont')
		tkconfigure(roiText, state='disabled')
	}
	...
	onNextRoi <- function(){
		...
		if (currRoi == 'All')
			...
			nextRoi <- match(currRoi, roiTable$Name) + 1
			if (nrow(roiTable > 1))
				tkconfigure(prevRoiButton, state='normal')
		}
		if (nextRoi != nrow(roiTable))
			tkconfigure(nextRoiButton, text='Next')
		else
			tkconfigure(nextRoiButton, state='disabled')
		myAssign('roiTable', roiTable, save.backup=FALSE)
		
		## Set zoom level to view ROI
		usr=roiTable[nextRoi, 2:5]
		p <- -140
		usr[1:2] <- rev(sort(newRange(usr[1:2], f = -p/100)))
		usr[3:4] <- rev(sort(newRange(usr[3:4], f = -p/100)))	
		setGraphics(usr=usr, roi.cex=c(.8, .8), roi.labelPos='center', 
				save.backup=FALSE, refresh=TRUE)
		showRoi(roiTable)
		currRoi <<- roiTable$Name[nextRoi]
		updateRoi()
	}
	nextRoiButton <- ttkbutton(roiFrame, text='View', width=12, 
			command=onNextRoi)
	...
	onPrevRoi <- function(){
		...
		##get previous ROI
		prevRoi <- match(currRoi, roiTable$Name) - 1
		if (prevRoi == 1)
			tkconfigure(prevRoiButton, state='disabled')
		if (prevRoi + 1 != nrow(roiTable))
			tkconfigure(nextRoiButton, state='normal')
		myAssign('roiTable', roiTable, save.backup=FALSE)
		
		## Set zoom level to view ROI
		usr=roiTable[prevRoi, 2:5]
		p <- -140
		usr[1:2] <- rev(sort(newRange(usr[1:2], f = -p/100)))
		usr[3:4] <- rev(sort(newRange(usr[3:4], f = -p/100)))	
		setGraphics(usr=usr, roi.cex=c(.8, .8), roi.labelPos='center', 
				save.backup=FALSE, refresh=TRUE)
		showRoi(roiTable)
		currRoi <<- roiTable$Name[prevRoi]
		updateRoi()
	}
	prevRoiButton <- ttkbutton(roiFrame, text='Previous', width=12, 
			state='disabled', command=onPrevRoi)
	
	##create assignment description text
	assignFrame <- ttklabelframe(tt, text='Assignment')
	assignText <- tktext(assignFrame, width=19, height=6, wrap='none', 
			font='TkDefaultFont', cursor='arrow',	state='disabled', 
			xscrollcommand=function(...) tkset(assignXscr, ...))
	assignXscr <- ttkscrollbar(assignFrame, orient='horizontal',
			command=function(...) tkxview(assignText, ...))
	
	##modify assignment text
	updateAssign <- function(){
		tkconfigure(assignText, state='normal')
		tkdelete(assignText, '1.0', 'end')
		tkinsert(assignText, '1.0', 'Current assignment:\n')
		currAssign <- assignments$table$Name[match(currId, 
						assignments$table$Comp_ID)]
		currAssign <- strsplit(currAssign, '.', fixed=TRUE)[[1]][1]
		tkinsert(assignText, '2.0', paste(currAssign, '\n', sep=''), 'assignName')
		tkinsert(assignText, '4.0', '\nCompound ID:\n', 'label')
		tkinsert(assignText, '5.0', currId, 'id')
		tcl(assignText, 'tag', 'configure', 'assignName', foreground='red', 
				font='TkFixedFont')
		tcl(assignText, 'tag', 'configure', 'id', foreground='red', 
				font='TkFixedFont')
		
		##configure ID text to display BMRB page for compound when clicked
		if (length(grep('bmse', currId))){
			tcl(assignText, 'tag', 'bind', 'id', '<Enter>', function(...){
						tcl(assignText, 'tag', 'configure', 'id', underline=TRUE, 
								foreground='blue')
						tkconfigure(assignText, cursor='hand2')
					})
			tcl(assignText, 'tag', 'bind', 'id', '<Leave>', function(...){
						tcl(assignText, 'tag', 'configure', 'id', underline=FALSE,
								foreground='red')
						tkconfigure(assignText, cursor='arrow')
					})
			bmrbPage <- paste('http://www.bmrb.wisc.edu/metabolomics/mol_summary/',
					'show_data.php?molName=', currAssign, '&id=', currId, '&whichTab=1', 
					sep='')
			tcl(assignText, 'tag', 'bind', 'id', '<Button-1>', function(...)
						browseURL(bmrbPage))
		}else{
			tcl(assignText, 'tag', 'bind', 'id', '<Enter>', function(...)
						tcl(assignText, 'tag', 'configure', 'id', underline=FALSE, 
								foreground='red'))
			tcl(assignText, 'tag', 'bind', 'id', '<Button-1>', '')
		}
		tkconfigure(assignText, state='disabled')
	}
	
	##create accept assignment button
	onAccept <- function(){
		i <- grep(currId, assignments$table$Comp_ID)
		assignments$table[i, 'Score'] <- 1
		myAssign('assignments', assignments, save.backup=FALSE)
		autoAssign()
		onNextAssign()
	}
	acceptButton <- ttkbutton(assignFrame, text='Accept', width=12, 
			command=onAccept)
	
	##create reject assignment button
	onReject <- function(){
		i <- grep(currId, assignments$table$Comp_ID)
		assignments$table[i, 'Score'] <- 0
		myAssign('assignments', assignments, save.backup=FALSE)
		autoAssign()
		onNextAssign()
	}
	rejectButton <- ttkbutton(assignFrame, text='Reject', width=12, 
			command=onReject)
	
	##create next assignment button
	onNextAssign <- function(){
		i <- match(currId, idList) + 1
		
		##close dialog if there are no more assignments to verify
		if (i > length(idList)){
			updateAA()
			closeVer()
			return(invisible())
	...
	nextAssignButton <- ttkbutton(assignFrame, text='Next', width=12, 
			command=onNextAssign)
	
	##create previous assignment button
	onPrevAssign <- function(){
		i <- match(currId, idList) - 1
		id <- idList[i]
		currId <<- id
		getAssign(id)
	}
	prevAssignButton <- ttkbutton(assignFrame, text='Previous', width=12, 
			command=onPrevAssign)
	
	##restore original plot settings when dialog is closed
	closeVer <- function(){
		fileFolder[[currentSpectrum]]$graphics.par <- prevGraphPar
		tryCatch({myAssign('fileFolder', fileFolder, save.backup=FALSE)
					myAssign('overlayList', prevOverlays, save.backup=FALSE)
					myAssign('roiTable', prevRoiTable, save.backup=FALSE)
					myAssign('globalSettings', prevGlobal, save.backup=FALSE)
					refresh()}, 
				error=function(er){})
		tkdestroy(tt)
	}
	tkwm.protocol(tt, 'WM_DELETE_WINDOW', closeVer)
	
	##add widgets to roiFrame
	tkgrid(roiFrame, column=1, row=1, pady=c(3, 5), padx=c(8, 0), sticky='nswe')
	tkgrid(roiText, column=1, columnspan=2, row=1, padx=5, pady=c(2, 0), 
			sticky='nswe')
	tkgrid(roiXscr, column=1, columnspan=2, row=2, padx=5, pady=c(0, 3), 
			sticky='we')	
	tkgrid(nextRoiButton, column=1, row=3, padx=c(4, 1), pady=3)
	tkgrid(prevRoiButton, column=2, row=3, padx=c(1, 6), pady=3)
	
	##make roiText stretch when window is resized
	tkgrid.columnconfigure(tt, 1, weight=1)
	tkgrid.rowconfigure(tt, 1, weight=1)
	tkgrid.columnconfigure(roiFrame, 1, weight=1)
	tkgrid.rowconfigure(roiFrame, 1, weight=1)
	tkgrid.columnconfigure(roiFrame, 2, weight=1)
	
	##add widgets to assignFrame
	tkgrid(assignFrame, column=1, row=2, padx=c(8, 0), sticky='nswe')
	tkgrid(assignText, column=1, columnspan=2, row=1, padx=5, pady=c(2, 0), 
			sticky='nswe')
	tkgrid(assignXscr, column=1, columnspan=2, row=2, padx=5, pady=c(0, 3), 
			sticky='we')	
	tkgrid(acceptButton, column=1, row=3, pady=3, padx=c(4, 1))
	tkgrid(rejectButton, column=2, row=3, pady=3, padx=c(1, 6))
	tkgrid(nextAssignButton, column=1, row=4, pady=c(0, 3), padx=c(4, 1))
	tkgrid(prevAssignButton, column=2, row=4, pady=c(0, 3), padx=c(1, 6))
	tkgrid(ttksizegrip(tt), column=2, row=3, sticky='se')
	
	##make assignText stretch when window is resized
	tkgrid.rowconfigure(tt, 2, weight=1)
	tkgrid.columnconfigure(assignFrame, 1, weight=1)
	tkgrid.rowconfigure(assignFrame, 1, weight=1)
	tkgrid.columnconfigure(assignFrame, 2, weight=1)
...
silentOpen <- function(fileList){
	...
	for (i in seq_along(fileList)){
		...
		##Read Sparky Header and file info from binary
		new.file <- try(ucsfHead(file.name=fileList[i], print.info=FALSE), 
				silent=TRUE)
...
## Add a standard to a library
## libPath - character string; full directory path to the standards library
## inFolder - list; data and file parameters for input spectrum.  This should 
##	match the	output format of ucsf2D().  Defaults to the current spectrum.
## rois - data.frame; an ROI table, defaults to the current ROI table.
## standName - character string; name for the standard being added to library
...
addToLib <- function(libPath, inFolder, rois=roiTable, standName, compID, 
	...
	##get library path
	if (missing(libPath)){
		libPath <- myDir(title = 'Select a standards library:')
		if (!nzchar(libPath))
			return(invisible())		
	}
	...
	##check if the selected library exists
	metaPath <- file.path(libPath, 'metadata.txt')
	recPath <- file.path(libPath, 'roiRecord.roi')
	hashPath <- file.path(libPath, 'hash.ucsf')
	existLib <- all(file.exists(c(libPath, metaPath, recPath, hashPath)))
	if (!existLib){
		unlink(libPath, recursive=TRUE)
		dir.create(libPath, recursive=TRUE)
	}
	
	##ensure the library contains spectra similar to the one being added
	if (existLib){
		metadata <- dget(metaPath)
		metaNuc <- metadata$Direct_nucleus
		if (nzchar(metadata$Indirect_nucleus))
			metaNuc <- c(metadata$Indirect_nucleus, metaNuc)
		if (length(metaNuc) != fp$number_dimensions)
			err(paste('You must select a library that contains spectra with the same', 
							'number of dimensions as the current spectrum.\nAlternatively,',
							'you may create a new library.'))
		if (!identical(metaNuc, fp$nucleus))
			err(paste('You must select a library that contains spectra with the same', 
							'nuclei as the current spectrum.\nAlternatively, you may create',
							'a new library.'))
	}
	
	##get standard name
	if (missing(standName)){
		standName <- myDialog('Standard name:', '', 'Custom Library')
		if (is.null(standName))
			return(invisible())
	}
	
	##get compound ID
	if (missing(compID))
		compID <- paste('usr', format(Sys.time(), '%H%M%m%d%y'), sep='')
	
	##print status message
	cat('\nAdding spectrum to library, please wait . . .\n')
	
	##create/append ROI record
	rois$Name <- rep(standName, nrow(rois))
	rois$Comp_ID <- rep(compID, nrow(rois))
	if (existLib){
		prevRec <- read.table(recPath, head=TRUE, sep='\t',	stringsAsFactors=FALSE)
		roiRec <- orderROI(rbind(prevRec, rois))[order(c(prevRec$Name, 
								rois$Name)), ]
	}else
		roiRec <- orderROI(rois)[order(rois$Name), ]
	write.table(roiRec, file=recPath, quote=FALSE, sep='\t', row.names=FALSE, 
			col.names=TRUE)
	
	##create RSD file for current spectrum
	rsdPath <- file.path(libPath, paste(standName, '[', compID, ']', '.rsd', 
					sep=''))
	createRsd(outPath=rsdPath, rois=rois)
	
	##create/append hash file
	roiSpec <- psuedoRsd(inFolder, returnType='folder')
	highRes <- interpSpec(roiSpec, returnType='folder', writeNoise=TRUE)
	matchRes <- smoothDat(highRes, returnType='folder', internal=TRUE)
	hashPath <- file.path(libPath, 'hash.ucsf')
	if (existLib)
		mathNMR(matchRes, inPath=hashPath, outPath=hashPath, internal=TRUE,
				normFun=as.list(c('threshold', 'none')))
	else
		mathNMR(matchRes, outPath=hashPath, normFun='threshold', internal=TRUE)
	
	##edit library metadata
	currDate <- as.character(Sys.Date())
	if (!existLib)
		metadata <- list(Created=currDate, Direct_nucleus=fp$nucleus[2], 
				Indirect_nucleus=fp$nucleus[1])
	metadata$Modified <- currDate
	dput(metadata, file=metaPath)
	
	##add standard to index file
	indexPath <- file.path(dirname(libPath), 'index.txt')
	if (file.exists(indexPath)){
		indexEntry <- t(c(basename(rsdPath), basename(libPath), currDate))
		write.table(indexEntry, indexPath, append=TRUE, quote=FALSE, sep='\t', 
				row.names=FALSE, col.names=FALSE)
	}
	
	##add standard info to library details
	if (!is.null(strPath)){
		detailsEntry <- t(getLibDetails(strPath))
		detailsPath <- file.path(libPath, 'details.txt')
		if (file.exists(detailsPath))
			write.table(detailsEntry, detailsPath, append=TRUE, quote=FALSE, sep='\t', 
					row.names=FALSE, col.names=FALSE)
		else
			write.table(detailsEntry, detailsPath, append=TRUE, quote=FALSE, sep='\t', 
					row.names=FALSE)
	...
	##add library to defaultSettings
	defaultSettings$libLocs <- unique(c(defaultSettings$libLocs, libPath))
	defaultSettings$searchLibs <- unique(c(defaultSettings$searchLibs, libPath))
	myAssign('defaultSettings', defaultSettings, save.backup=FALSE)
	writeDef()
	
	##print completion message
	cat('\n"', standName, '"', ' was successfully added to:\n ', libPath, '\n', 
			sep='')
	
	return(invisible())
}

## Remove a standard from a library
rmFromLib <- function(libPath, standName){
	...
	if (file.exists(recPath))
		roiRec <- read.table(recPath, head=TRUE, sep='\t', stringsAsFactors=FALSE)
	
	##remove library if it doesn't contain a valid ROI record
	if (is.null(roiRec) || !length(roiRec) || !nrow(roiRec)){
		usrSel <- myMsg(paste('The selected library does not contain any valid', 
						'ROIs.\nWould you like to delete the library?'), 'yesno')
		if (usrSel == 'yes')
			unlink(libPath, recursive=TRUE)
		searchMatch <- match(libPath, defaultSettings$searchLibs)
		if (!is.na(searchMatch))
			defaultSettings$searchLibs <- defaultSettings$searchLibs[-searchMatch]
		defaultSettings$libLocs <- defaultSettings$libLocs[-match(libPath, 
						defaultSettings$libLocs)]
		myAssign('defaultSettings', defaultSettings, FALSE)
		writeDef()
		return(invisible())
	}
	
	##get compound names and IDs within library
	compNames <- sapply(strsplit(roiRec$Name, '.', fixed=TRUE), function(x) 
				x[1])
	fullNames <- unique(paste(compNames, ' (', roiRec$Comp_ID, ')', 
					sep=''))
	ids <- sapply(strsplit(fullNames, ' (', fixed=TRUE), function(x) x[2])
	ids <- sapply(strsplit(ids, ')', fixed=TRUE), function(x) x[1])
	
	##display selection window with all available compounds from the library
	usrSel <- mySelect(fullNames, title='Select standard(s):', index=TRUE, 
			multi=TRUE, preselect=fullNames[1])
	if (!nzchar(usrSel[1]))
		return(invisible())
	
	##remove library if all available compounds are selected
	if (length(usrSel) == length(fullNames)){
		usrSel <- myMsg(paste('The selected library will be deleted.\nDo you wish',
						'to continue?'), 'okcancel', icon='warning')
		if (usrSel == 'cancel')
			return(invisible())
		unlink(libPath, recursive=TRUE)
		searchMatch <- match(libPath, defaultSettings$searchLibs)
		...
		myAssign('defaultSettings', defaultSettings, FALSE)
		writeDef()
		cat('\n', '"', libPath, '" has been removed.\n', sep='')
		return(invisible())
	}
	ids <- ids[usrSel]
	
	##remove library files corresponding to each compound selected
	rsdFiles <- list.files(libPath, pattern='.rsd', full.names=TRUE)
	hashFolder <- NULL
	for (id in ids){
		
		##remove compound from the ROI record
		roiIndices <- grep(id, as.character(roiRec$Comp_ID))
		rois <- roiRec[roiIndices, ]
		roiRec <- roiRec[-roiIndices, ]
		
		##create a hash resolution spectrum from the RSD for the compound
		rsdPath <- rsdFiles[grep(paste('[', id, ']', sep=''), rsdFiles, 
						fixed=TRUE)]
		roiSpec <- psuedoRsd(inPath=rsdPath, rois=rois, returnType='folder')
		highRes <- interpSpec(roiSpec, returnType='folder', writeNoise=TRUE)
		matchRes <- smoothDat(highRes, returnType='folder', internal=TRUE)
		
		##add hash for current compound to hash for previous compound
		if (!is.null(hashFolder)){
			matchRes <- list(matchRes, hashFolder)
			normFun <- as.list(c('threshold', 'none'))
		}else
			normFun <- 'threshold'
		hashFolder <- mathNMR(matchRes, returnType='folder', normFun=normFun, 
				internal=TRUE)
		
		##remove the RSD file for compound
		file.remove(rsdPath)
	}
	
	##subtract combined hash from library hash
	hashPath <- file.path(libPath, 'hash.ucsf')
	mathNMR(inPath=hashPath, inFolder=hashFolder, outPath=hashPath, 
			mathFun='subtract', internal=TRUE)
	
	##replace ROI record in library
	write.table(roiRec, file=recPath, quote=FALSE, sep='\t', row.names=FALSE,
			col.names=TRUE)
}

## Standard library customization GUI
cl <- function(){
	
	##check for an ROI table
	wc()
	if (is.null(roiTable) || !length(roiTable) || !nrow(roiTable))
		err(paste('You must create ROIs for the current spectrum before adding',
						'the spectrum to a library'))
	currTable <- roiTable
	...
	if (is.null(dlg))
		return(invisible())
	tkwm.title(dlg, 'Custom Libraries')
	tkwm.resizable(dlg)
	tkfocus(dlg)
	tkwm.deiconify(dlg)
	metadata <- NULL
	
	##create standard name widgets
	standFrame <- ttklabelframe(dlg, text='Standard')
	standNameLabel <- ttklabel(standFrame, text='Name:')
	standName <- strsplit(basename(currentSpectrum), '.', fixed=TRUE)[[1]][1]
	standNameVal <- tclVar(standName)
	standNameEntry <- ttkentry(standFrame, textvariable=standNameVal, width=15)
	standNameDescLabel <- ttklabel(standFrame, 
			text='*ROIs will be renamed to match')
	
	##create ROI information widgets
	roiInfoFrame <- ttkframe(standFrame, relief='sunken')
	numRoisLabel <- ttklabel(roiInfoFrame, text=paste('Regions of Interest:    ', 
					nrow(currTable)))
	unassigned <- grep('?', currTable$Assignment, fixed=TRUE)
	unassigned <- c(unassigned, grep('NA', currTable$Assignment))
	unassigned <- c(unassigned, 
			which(suppressWarnings(is.na(currTable$Assignment))))
	assigned <- nrow(currTable) - length(unassigned)
	numAssignLabel <- ttklabel(roiInfoFrame, 
			text=paste('Assigned Peaks:        ', assigned))
	
	##create edit ROI table button
	onRoiEdit <- function(){
		newTable <- re(currTable)
		if (!is.null(newTable)){
			myAssign('roiTable', newTable)
			onMouse()
		}
	}
	editRoiButton <- ttkbutton(standFrame, text='Edit ROI Table', width=15,
			command=onRoiEdit)
	standardWidgets <- list(standNameLabel, standNameEntry, standNameDescLabel, 
			numRoisLabel, numAssignLabel, editRoiButton)
	
	##create new library radiobutton
	libFrame <- ttklabelframe(dlg, text='Library')
	newLibFrame <- ttkframe(libFrame)
	libRadioVal <- tclVar('new')
	onNewLib <- function(){
		
		##configure new library widgets
		for (i in list(newNameLabel, newNameEntry))
			tkconfigure(i, state='normal')
		if (nzchar(tclvalue(newNameVal))){
			tkconfigure(newLocLabel, state='normal')
			tkconfigure(newLocEntry, state='readonly')
			tkconfigure(newBrowseButton, state='normal')
			if (nzchar(tclvalue(newLocVal))){
				for (i in list(newMetaButton, newMetaLabel, newAddButton, newAddLabel))
					tkconfigure(i, state='normal')
			}
		}else{
			for (i in list(newLocLabel, newLocEntry, newBrowseButton, newMetaButton, 
					newMetaLabel, newAddButton, newAddLabel))
				tkconfigure(i, state='disabled')
		}
		
		##disable existing library widgets
		tkconfigure(existLibCombo, foreground='grey')
		for (i in existWidgets)
			tkconfigure(i, state='disabled')
	}
	newLibRadio <- ttkradiobutton(newLibFrame, variable=libRadioVal, value='new',
			text='Create new library', command=onNewLib)
	
	##create new library name widgets
	newNameFrame <- ttkframe(newLibFrame)
	newNameLabel <- ttklabel(newNameFrame, text=paste('Library name (files will', 
					'be stored in this directory):'))
	newNameVal <- tclVar('')
	onNewNameEdit <- function(){
		onNewLib()
		return(tclVar(1))
	}
	newNameEntry <- ttkentry(newNameFrame, textvariable=newNameVal,	width=14, 
			validate='all', validatecommand=onNewNameEdit)
	
	##create new library location label and entry box
	newLocFrame <- ttkframe(newLibFrame)
	newLocLabel <- ttklabel(newLocFrame, text='Location:', state='disabled')
	newLocVal <- tclVar('')
	newLocEntry <- ttkentry(newLocFrame, textvariable=newLocVal, width=33, 
			state='disabled')
	
	##create browse button
	onBrowseNew <- function(){
		
		##get library path
		usrDir <- myDir(parent=dlg, title='Select library location:')
		if (!nzchar(usrDir))
			return(invisible())
		
		##configure widgets
		tclvalue(newLocVal) <- usrDir
		for (i in list(newMetaButton, newMetaLabel, newAddButton, newAddLabel))
			tkconfigure(i, state='normal')
	}
	newBrowseButton <- ttkbutton(newLocFrame, text='Browse...', width=9, 
			state='disabled',	command=onBrowseNew)
	
	##create edit new library metadata button
	newMetaFrame <- ttkframe(newLibFrame)
	onNewMeta <- function(){
		metaPath <- paste(tclvalue(newLocVal), tclvalue(newNameVal), 'metadata.txt', 
				sep='/')
		metadata <- metaGui(dlg, metaPath, metadata)
		if (!is.null(metadata))
			metadata <<- metadata
	}
	newMetaButton <- ttkbutton(newMetaFrame, text='Edit Metadata', width=15, 
			state='disabled', command=onNewMeta)
	newMetaLabel <- ttklabel(newMetaFrame, state='disabled',
			text='Edit details pertaining to the library\'s contents')
	
	##create add standard to new library button
	newAddFrame <- ttkframe(newLibFrame)
	onNewAdd <- function(){
		
		##get library path
		libPath <- file.path(tclvalue(newLocVal), tclvalue(newNameVal))
		
		##get standard name
		standName <- tclvalue(standNameVal)
		if (!nzchar(standName))
			err('You must provide a name for the standard.', dlg)
		
		##add spectrum to library (ROIs, RSD, and hash)
		tkconfigure(dlg, cursor='watch')
	...
	newAddButton <- ttkbutton(newAddFrame, text='Add Standard', width=15, 
			state='disabled', command=onNewAdd)
	newAddLabel <- ttklabel(newAddFrame, state='disabled',
			text='Create library and add the current spectrum')
	newWidgets <- list(newNameLabel, newNameEntry, newLocLabel, newLocEntry, 
			newBrowseButton, newMetaButton, newMetaLabel, newAddButton, newAddLabel)
	
	##create add to existing library radiobutton
	existLibFrame <- ttkframe(libFrame)
	onExistLib <- function(){
		
		##configure existing library widgets
		for (i in existWidgets){
			tkconfigure(i, state='normal')
			tkconfigure(existLibCombo, state='readonly', foreground='black')
		}
		
		##disable new library widgets
		for (i in newWidgets)
			tkconfigure(i, state='disabled')
	}
	existLibRadio <- ttkradiobutton(existLibFrame, variable=libRadioVal,
			value='exist', command=onExistLib,
			text=paste('Edit existing custom library (the default library may not be', 
					'modified)'))
	if (!length(defaultSettings$libLocs) > 1)
		tkconfigure(existLibRadio, state='disabled')
	
	##create library selection combobox
	existSelFrame <- ttkframe(existLibFrame)
	existSelLabel <- ttklabel(existSelFrame, text='Select library:')
	searchLibs <- defaultSettings$searchLibs[-1]
	selectedLib <- tclVar(searchLibs[1])
	existLibCombo <- ttkcombobox(existSelFrame, textvariable=selectedLib, 
			height=length(searchLibs), width=37, values=paste('{', searchLibs, '}', 
					sep=''), exportselection=FALSE)
	
	##create add standard to existing library button
	existAddFrame <- ttkframe(existLibFrame)
	onExistAdd <- function(){
		
		##get library path
		libPath <- tclvalue(selectedLib)
	...
	existAddButton <- ttkbutton(existAddFrame, text='Add', width=12, 
			state='disabled', command=onExistAdd)
	existAddLabel <- ttklabel(existAddFrame, state='disabled',
			text='Add the current spectrum to the selected library')
	
	##create remove from existing library button
	existRemoveFrame <- ttkframe(existLibFrame)
	onExistRemove <- function(){
		
		##get library path
		libPath <- tclvalue(selectedLib)
		
		##remove spectrum from library (ROIs, RSD, and hash)
		tkconfigure(dlg, cursor='watch')
	...
	existRemoveButton <- ttkbutton(existRemoveFrame, text='Remove...', 
			width=12, command=onExistRemove)
	existRemoveLabel <- ttklabel(existRemoveFrame,
			text='Remove spectra from the selected library')
	existWidgets <- list(existSelLabel, existLibCombo, existAddButton, 
			existAddLabel, existRemoveButton, existRemoveLabel)
	
	##updates ROI info whenever the mouse enters the GUI
	onMouse <- function(){
		
		##check for custom libraries
		if (length(defaultSettings$libLocs) <= 1){
			tkinvoke(newLibRadio)
			tkconfigure(existLibRadio, state='disabled')
		}else{
			tkconfigure(existLibRadio, state='normal')
		}
		
		##check for changes to roiTable
		if (is.null(roiTable) || !length(roiTable) || !nrow(roiTable)){
			if (length(defaultSettings$libLocs) <= 1){
				tkdestroy(dlg)
				err(paste('You must create ROIs for the current spectrum before adding',
			...
			tkinvoke(existLibRadio)
			for (i in standardWidgets)
				tkconfigure(i, state='disabled')
			for (i in list(newLibRadio, existAddButton, existAddLabel))
				tkconfigure(i, state='disabled')
			return(invisible())
		}else{
			if (identical(currTable, roiTable))
				return(invisible())
		}
		currTable <<- roiTable
		
		##configure GUI
		tkconfigure(newLibRadio, state='normal')
		for (i in standardWidgets)
			tkconfigure(i, state='normal')
		tkconfigure(numRoisLabel, text=paste('Regions of Interest:    ', 
						nrow(currTable)))
		unassigned <- grep('?', currTable$Assignment, fixed=TRUE)
		unassigned <- c(unassigned, 
				suppressWarnings(which(is.na(currTable$Assignment))))
		assigned <- nrow(currTable) - length(unassigned)
		tkconfigure(numAssignLabel, text=paste('Assigned Peaks:        ', 
						assigned))
	}
	tkbind(dlg, '<Enter>', onMouse)
	tkbind(dlg, '<FocusIn>', onMouse)
	
	##add widgets to standFrame
	tkgrid(standFrame, row=1, column=1, padx=8, pady=4, sticky='nswe')
	tkgrid(standNameLabel, row=1, column=1, padx=c(13, 0), pady=2)
	tkgrid(standNameEntry, row=1, column=2, padx=3, pady=2, sticky='w')
	tkgrid(standNameDescLabel, row=1, column=3, pady=2, sticky='w')
	
	tkgrid(roiInfoFrame, row=2, column=1, columnspan=2, padx=c(15, 9), pady=6, 
			sticky='w')
	tkgrid(numRoisLabel, row=1, column=1, padx=c(6, 8), pady=c(10, 3), sticky='w')
	tkgrid(numAssignLabel, row=2, column=1, padx=c(6, 8), pady=c(3, 10), 
			sticky='w')
	tkgrid(editRoiButton, row=1, rowspan=2, column=3, pady=c(20, 0), sticky='w')
	
	##add widgets to newLibFrame
	tkgrid(libFrame, row=2, column=1, padx=8, pady=4, sticky='nswe')
	tkgrid(newLibFrame, row=1, column=1, pady=2, padx=c(4, 8), sticky='nswe')
	tkgrid(newLibRadio, row=1, column=1, columnspan=3, padx=2, pady=2, sticky='w')
	
	tkgrid(newNameFrame, row=2, column=1, padx=c(10, 0), pady=2, sticky='w')
	tkgrid(newNameLabel, row=1, column=1)
	tkgrid(newNameEntry, row=1, column=2, padx=3, sticky='w')
	
	tkgrid(newLocFrame, row=3, column=1, padx=c(10, 0), pady=2, sticky='w')
	tkgrid(newLocLabel, row=1, column=1)
	tkgrid(newLocEntry, row=1, column=2, padx=2, sticky='w')
	tkgrid(newBrowseButton, row=1, column=3, padx=3, sticky='w')
	
	tkgrid(newMetaFrame, row=4, columnspan=3, padx=c(10, 0), pady=2, sticky='w')
	tkgrid(newMetaButton, row=1, column=1)
	tkgrid(newMetaLabel, row=1, column=2, padx=c(3, 0), sticky='w')
	
	tkgrid(newAddFrame, row=5, columnspan=3, padx=c(10, 0), pady=2, sticky='w')
	tkgrid(newAddButton, row=1, column=1)
	tkgrid(newAddLabel, row=1, column=2, padx=c(3, 0), sticky='w')
	
	##add widgets to existLibFrame
	tkgrid(existLibFrame, row=2, column=1, pady=12, padx=c(4, 8), sticky='nswe')
	tkgrid(existLibRadio, row=1, column=1, columnspan=2, padx=2, pady=2, 
			sticky='w')
	
	tkgrid(existSelFrame, row=2, column=1, padx=c(12, 5), pady=2, sticky='w')
	tkgrid(existSelLabel, row=1, column=1)
	tkgrid(existLibCombo, row=1, column=2, padx=c(5, 0), sticky='w')
	
	tkgrid(existAddFrame, row=3, column=1, padx=c(10, 0), pady=2, sticky='w')
	tkgrid(existAddButton, row=1, column=1)
	tkgrid(existAddLabel, row=1, column=2, padx=c(5, 0), sticky='w')
	
	tkgrid(existRemoveFrame, row=4, column=1, padx=c(10, 0), pady=2, sticky='w')
	tkgrid(existRemoveButton, row=1, column=1, pady=c(0, 6))
	tkgrid(existRemoveLabel, row=1, column=2, padx=c(5, 0), pady=c(0, 6), 
			sticky='w')
	...
	if (is.null(roiTable) || !length(roiTable) || !nrow(roiTable)){
		...
		for (i in standardWidgets)
			tkconfigure(i, state='disabled')
		tkconfigure(newLibRadio, state='disabled')
		tkconfigure(existAddButton, state='disabled')
	}else
		tkinvoke(newLibRadio)
...
## Metadata edit GUI
metaGui <- function(parent, metaPath, metadata=NULL){
	
	##create main window
	current <- wc()
	tt <- tktoplevel(parent=parent)
	tcl('wm', 'attributes', tt, topmost=TRUE)
	tkwm.title(tt, 'Edit Metadata')
	...
	metaNames <- c('Organism', 'Sample', 'Direct_nucleus', 'Indirect_nucleus', 
			'Experiment', 'Field_strength', 'Solvent', 'pH', 'Created', 'Modified', 
			'Comments')
	if (file.exists(metaPath)){
		metadata <- dget(metaPath)
	}else if (is.null(metadata)){
		metadata <- as.list(rep('', length(metaNames)))
		names(metadata) <- metaNames
	}
	
	##insert nuclei if missing
	if (!nzchar(metadata$Direct_nucleus) || !length(metadata$Direct_nucleus))
		metadata$Direct_nucleus <- fileFolder[[current]]$file.par$nucleus[2]
	if (!nzchar(metadata$Indirect_nucleus) || !length(metadata$Indirect_nucleus))
		metadata$Indirect_nucleus <- fileFolder[[current]]$file.par$nucleus[1]
	
	##insert created and modified dates if missing
	currDate <- as.character(Sys.Date())
	if (!nzchar(metadata$Created) || !length(metadata$Created))
		metadata$Created <- currDate
	if (!nzchar(metadata$Modified) || !length(metadata$Modified))
		metadata$Modified <- currDate
	
	##create leftFrame and widgets
	leftLabels <- leftEntries <- list()
	leftIndices <- seq(1, length(metaNames) - 2, 2)
	leftText <- metaNames[leftIndices]
	entryText <- metadata[leftIndices]
	leftFrame <- ttkframe(tt)
	tkgrid(leftFrame, row=1, column=1, padx=6, pady=5)
	for (i in seq_along(leftText)){
		
		##create widgets
		leftLabels[[i]] <- ttklabel(leftFrame, text=paste(leftText[i], ':', 
						sep=''))
		leftEntries[[i]] <- ttkentry(leftFrame, width=20)
		currEntry <- entryText[[i]]
		if (is.null(currEntry))
			currEntry <- ''
		tkinsert(leftEntries[[i]], 0, currEntry)
		
		##grid widgets
		tkgrid(leftLabels[[i]], row=i, column=1, padx=2, pady=4, sticky='e')
		tkgrid(leftEntries[[i]], row=i, column=2, padx=2, pady=4, sticky='w')
	}
	
	##create rightFrame and widgets
	rightLabels <- rightEntries <- list()
	rightIndices <- seq(2, length(metaNames) - 1, 2)
	rightText <- metaNames[rightIndices]
	entryText <- metadata[rightIndices]
	rightFrame <- ttkframe(tt)
	tkgrid(rightFrame, row=1, column=2, padx=c(0, 9), pady=5)
	for (i in seq_along(rightText)){
		
		##create widgets
		rightLabels[[i]] <- ttklabel(rightFrame, text=paste(rightText[i], ':', 
						sep=''))
		rightEntries[[i]] <- ttkentry(rightFrame, width=20)
		currEntry <- entryText[[i]]
		if (is.null(currEntry))
			currEntry <- ''
		tkinsert(rightEntries[[i]], 0, currEntry)
		
		##grid widgets
		tkgrid(rightLabels[[i]], row=i, column=1, padx=2, pady=4, sticky='e')
		tkgrid(rightEntries[[i]], row=i, column=2, padx=2, pady=4, sticky='w')
	}
	
	##disable entries that should not be editable
	noEdit <- c('Direct_nucleus', 'Indirect_nucleus', 'Created', 'Modified')
	for (i in seq_along(leftText)){
		if (leftText[i] %in% noEdit)
			tkconfigure(leftEntries[[i]], state='disabled')
	}
	for (i in seq_along(rightText)){
		if (rightText[i] %in% noEdit)
			tkconfigure(rightEntries[[i]], state='disabled')
	}
	
	##create comments label and text widget
	commentsFrame <- ttkframe(tt)
	commentsLabel <- ttklabel(commentsFrame, text='Comments:')
	commentsText <- tktext(commentsFrame, wrap='word', width=50, height=6)
	
	##remove empty values at the end of the comments and insert text
	comments <- unlist(strsplit(metadata$Comments, '\n', fixed=TRUE))
	if (length(comments)){
		repeat{
			if (length(comments) && !nzchar(comments[length(comments)]))
	...
	if (!is.null(metadata$Comments))
		tkinsert(commentsText, '1.0', metadata$Comments)
	
	##create ok button
	bottomFrame <- ttkframe(tt)
	onOk <- function(){
		
		##get text from entries on the left
		for (i in seq_along(leftIndices))
			metadata[[leftIndices[i]]] <- tclvalue(tkget(leftEntries[[i]]))
		
		##get text from entries on the right
		for (i in seq_along(rightIndices))
			metadata[[rightIndices[i]]] <- tclvalue(tkget(rightEntries[[i]]))
		
		##get text from the comments text widget
		metadata$Comments <- tclvalue(tkget(commentsText, '1.0', 'end'))
		
		##remove empty values at the end of the comments
		comments <- unlist(strsplit(metadata$Comments, '\n', fixed=TRUE))
		if (length(comments)){
			repeat{
				if (length(comments) && !nzchar(comments[length(comments)]))
	...
	okButton <- ttkbutton(bottomFrame, text='OK', command=onOk)
	...
	##add widgets to commentsFrame
	tkgrid(commentsFrame, row=2, column=1, columnspan=2, padx=c(6, 9), pady=4, 
			sticky='nswe')
	tkgrid(commentsLabel, row=1, column=1, padx=2, pady=2, sticky='w')
	tkgrid(commentsText, row=2, column=1, padx=c(12, 2))
	
	##add widgets to bottomFrame
	tkgrid(bottomFrame, row=3, column=2, pady=6, padx=6, sticky='se')
	tkgrid(okButton, row=1, column=1, padx=4)
	tkgrid(cancelButton, row=1, column=2, padx=4)
	
	## configure the toplevel
	tkfocus(commentsText)
	if (as.logical(tkwinfo('viewable', tt)))
		tkgrab.set(tt)
	tkwait.window(tt)
...
## Read and store an NMR-STAR file as a list object
## inFile - character string; full file path for str file
## frames - character string or vector; str files are separated into sections
##	called frames.  Each frame is denoted by "save_" and then a name for the
##	frame.  This argument limits the return value to the desired frames.
parseStr <- function(inFile, frames='ALL'){
	
	##get str file
	if (missing(inFile)){
		inFile <- myOpen('str', list(str='NMR-STAR File'), 
				title='Select an NMR-STAR file')
		if (!length(inFile))
			return(invisible())
	}
	
	##read STAR file, one line at a time
	strCon <- file(inFile, 'r')
	strList <- frameList <- list()
	isFrame <- isLoop <- isFreeText <- skipFrame <- FALSE
	frameNames <- loopRow <- loopTable <- infoNames <- colNames <- NULL
	strText <- readLines(strCon, warn=FALSE)
	for (n in seq_along(strText)){
		currLine <- strText[n]
		
		##check for frame start delimiter
		if (length(currLine) && length(grep('^save_.+', currLine))){
			frameName <- strsplit(currLine, '^save_')[[1]][2]
			frameNames <- c(frameNames, frameName)
			skipFrame <- !(frames == 'ALL' || frameName %in% frames)
			isFrame <- TRUE
			next
		}
		if (skipFrame)
		...
		##skip any text not stored within a frame
		if (!isFrame)
			next
		
		##check for free text delimiter
		if (length(grep('^ *; *$', currLine))){
		...
		if (isFreeText){
			...
			##store frame info free text
			if (!isLoop){
				frameList <- c(frameList, list(info=currLine))
				next
			}
			
			##store loop info free text
			loopRow <- c(loopRow, currLine)
			next
		}
		
		##skip blank lines
		if (!nzchar(currLine) || length(grep('^ +$', currLine)))
			next
		
		##check for loop start delimiter
		if (length(grep('^ *loop_ *$', currLine))){
			isLoop <- TRUE
			next
		}
		
		##check for loop end delimiter
		if (length(grep('^ *stop_ *$', currLine))){
			loopName <- strsplit(strsplit(colNames[1], '.', fixed=TRUE)[[1]][1], 
					'^ *_')[[1]][2]
			infoNames <- c(infoNames, loopName)
			loopTable <- as.data.frame(loopTable, stringsAsFactors=FALSE)
			names(loopTable) <- sub('.*\\.', '', colNames)
			rownames(loopTable) <- NULL
			frameList <- c(frameList, list(loop=loopTable))
			isLoop <- FALSE
			loopRow <- loopTable <- colNames <- NULL
			next
		}
		
		##check for frame end delimiter
		if (length(grep('^ *save_ *$', currLine))){
			names(frameList) <- infoNames
			strList <- c(strList, list(frame=frameList))
			isFrame <- FALSE
			infoNames <- NULL
			frameList <- list()
			next
		...
		if (!isLoop){
			...
			##check for lines preceding free text
			testLine <- strsplit(currLine, ' +')[[1]]
			if (length(testLine) < 3){
				infoNames <- c(infoNames, sub('.*\\.', '', testLine[2]))
				next
			}
			
			##parse lines containing quoted text
			quoteChar <- "'"
			quoteMatch <- length(gregexpr(quoteChar, currLine)[[1]]) > 1
			if (!quoteMatch){
				quoteChar <- '"'
				quoteMatch <- length(gregexpr(quoteChar, currLine)[[1]]) > 1
			}
			if (quoteMatch){
				infoName <- strsplit(currLine, ' +')[[1]][2]
				infoNames <- c(infoNames, sub('.*\\.', '', infoName))
				infoVal <- strsplit(currLine, quoteChar)[[1]][2]
			}else{
				
				##parse standard info line
				currLine <- strsplit(currLine, ' +')[[1]]
				infoNames <- c(infoNames, sub('.*\\.', '', currLine[2]))
				infoVal <- currLine[3]
			}
			
			##store frame info
			frameList <- c(frameList, list(infoName=infoVal))
			next
		}
		
		##get loop info names (colum names for the table described by the loop)
		if (length(grep('^ *_', currLine))){
			colNames <- c(colNames, strsplit(currLine, ' +')[[1]][2])
			next
		}
		
		##get loop info (a single row in the table)
		currLine <- strsplit(currLine, ' +')[[1]]
		
		##remove leading space
		if (!nzchar(currLine[1]))
			currLine <- currLine[-1]
		
		##unsplit quoted text
		quoteChar <- "'"
		quoteMatches <- grep(quoteChar, currLine)
		if (length(quoteMatches) < 2){
			quoteChar <- '"'
			quoteMatches <- grep(quoteChar, currLine)
		}
		if (length(quoteMatches) > 1){
			for (i in seq_along(quoteMatches)){
				if (!i %% 2)
					next
				matchRange <- quoteMatches[i]:quoteMatches[i + 1]
				quoteSub <- paste(currLine[matchRange], collapse=' ')
				quoteSub <- gsub(quoteChar, '', quoteSub)
				currLine[matchRange] <- gsub(quoteChar, '', quoteSub)
				currLine[(which(currLine == quoteSub)[-1])] <- NA
			}
			currLine <- as.vector(na.omit(currLine))
		}
		
		##store row
		if (length(currLine) == length(colNames))
			loopTable <- rbind(loopTable, currLine)
		else{
			loopRow <- c(loopRow, currLine)
			if (length(loopRow) == length(colNames))
				loopTable <- rbind(loopTable, loopRow)
		}
	}
	close(strCon)
	names(strList) <- frameNames
...
## dispDSS - boolean; indicates whether or not a peak marker should be displayed
##	at (0, 0)
getStrPeaks <- function(strFile, exp='1H_13C_HSQC', dispDSS=TRUE){
	
	##read NMR-STAR file
	if (missing(strFile))
		strList <- parseStr()
	else
		strList <- parseStr(strFile)
	
	##check for peak list
	peakFrame <- paste('spectral_peak', exp, sep='_')
	if (!peakFrame %in% names(strList)){
		posExps <- c('1H', '13C', 'DEPT_90', 'DEPT_135', 'HSQC', 'TOCSY', 'HMBC', 
				'COSY')
		newExp <- NA
		for (i in posExps){
			if (length(grep(i, exp)))
				newExp <- i
		}
		peakFrame <- paste('spectral_peak', newExp, sep='_')
	}
	strPeaks <- strList[[peakFrame]]$Assigned_peak_chem_shift
	if (is.null(strPeaks))
		stop(paste('"', strFile, '" does not contain a peak list for experiment ', 
						exp, sep=''), call.=FALSE)
	
	##get unique chemical shifts
	swap <- as.numeric(strPeaks[1, 'Val']) > as.numeric(strPeaks[2, 'Val'])
	w1Shifts <- w2Shifts <- NULL
	ids <- unique(strPeaks$Peak_ID)
	for (i in ids){
		shifts <- unique(strPeaks[which(strPeaks$Peak_ID == i), 'Val'])
		if (swap){
			w1Shifts <- c(w1Shifts, shifts[1])
			w2Shifts <- c(w2Shifts, shifts[2])
		}else{
			w1Shifts <- c(w1Shifts, shifts[2])
			w2Shifts <- c(w2Shifts, shifts[1])
		}
	}
	
	##get assignments
	assignments <- NULL
	for (i in seq_along(w2Shifts)){
		currPeak <- strPeaks[which(strPeaks$Peak_ID == i), ]
		assignments <- c(assignments, paste(unique(currPeak$Atom_ID), collapse=','))
	}
	assignments <- paste('P', ids, '_', assignments, sep='')
	
	##format peak list
	peakList <- data.frame(cbind(1:length(w2Shifts), w1Shifts, w2Shifts, 
					rep(0, length(w2Shifts)), assignments), stringsAsFactors=FALSE)
	if (dispDSS){
		peakList <- rbind(peakList, c(length(w2Shifts) + 1, 0, 0, 0, 'DSS'))
	}
	colnames(peakList) <- c('Index', 'w1', 'w2', 'Height', 'Assignment')
	for (i in 1:(ncol(peakList) - 1))
		mode(peakList[, i]) <- 'numeric'
	
	return(peakList)
}


## Extracts the necessary information from an NMR-STAR file for library 
##	inclusion
## strFile - character string; full file path for NMR-STAR file
## exp - character string; the name of the desired experiment, must match one
##	of the experiment names in the NMR-STAR file
getLibDetails <- function(strFile, exp='1H_13C_HSQC'){
	...
	##check experiment
	Experiment_fileFrame <- strList$experiment_list$Experiment_file
	Experiment_fileFrame <- Experiment_fileFrame[grep('Time-domain', 
					Experiment_fileFrame$Type), ]
	exps <- Experiment_fileFrame$Name
	exps <- sub('/*', '', exps, fixed=TRUE)
	exps <- basename(exps)
	if (!exp %in% exps){
		exp <- mySelect(exps, title='Select an experiment:')
		if (!nzchar(exp))
			return(invisible())
	}
	
	##get experiment info
	expIndex <- match(exp, exps)
	expPath <- Experiment_fileFrame$Name[expIndex]
	expID <- as.numeric(Experiment_fileFrame[expIndex, 'Experiment_ID'])
	expName <- strList$experiment_list$Experiment$Name[expID]
	expName <- gsub('\"', '', expName, fixed=TRUE)
	sampleID <- strList$experiment_list$Experiment$Sample_ID[expID]
	sampleCon <- 
			strList$experiment_list$Experiment$Sample_condition_list_label[expID]
	sampleCon <- sub('$', '', sampleCon, fixed=TRUE)
	
	##get entry info
	entryID <- strList$entry_information$ID
	entryTitle <- strList$entry_information$Title
	relDate <- strList$entry_information$Last_release_date
	
	##get field strength
	frameCats <- sapply(strList, function(x) x[[1]])
	fieldStrength <- strList[[match('NMR_spectrometer', 
					frameCats)]]$Field_strength
	
	##get entity label
	entLabel <- strList$assembly$Entity_assembly$Entity_label
	
	##get sample info
	sampleFrameName <- paste('sample', sampleID, sep='_')
	sampleFrame <- strList[[sampleFrameName]]
	sampleComp <- sampleFrame$Sample_component
	entIndex <- match(entLabel, sampleComp$Entity_label)
	
	##get sample concentration
	conc <- sampleComp$Concentration_val[entIndex]
	conc <- paste(conc, sampleComp$Concentration_val_units[entIndex])
	
	##get solvent and buffer
	solvent <- sampleComp$Mol_common_name[match('Solvent', sampleComp$Type)]
	buffer <- sampleComp$Mol_common_name[match('Buffer', sampleComp$Type)]
	
	##get chemical shift reference and concentration
	refIndex <- match('Reference', sampleComp$Type)
	ref <- sampleComp$Mol_common_name[refIndex]
	refConc <- sampleComp$Concentration_val[refIndex]
	refConc <- paste(refConc, sampleComp$Concentration_val_units[refIndex], 
			sep='')
	ref <- paste(ref, ' (', refConc, ')', sep='')
	
	##get sample conditions
	sampleCon <- strList[[sampleCon]]$Sample_condition_variable
	pH <- sampleCon$Val[match('pH', sampleCon$Type)]
	temp <- sampleCon$Val[match('temperature', sampleCon$Type)]
	
	##format return value
	details <- c(entryID, entryTitle, expID, expName, expPath, fieldStrength, 
			sampleID, conc, solvent, buffer, ref, pH, temp, relDate)
	names(details) <- c('ID', 'Title', 'Experiment ID', 'Experiment', 
			'Experiment Path', 'Field Strength', 'Sample ID', 'Concentration', 
			'Solvent', 'Buffer', 'Reference', 'pH', 'Temperature', 'Last Release')
	
	return(details)
}

## Manual assignment GUI, used for assigning BMRB standards to libraries 
ma <- function(){
	...
	if (is.null(dlg))
		return(invisible())
	tkwm.title(dlg, 'Manual Assign')
	tkfocus(dlg)
	tkwm.deiconify(dlg)
	
	##create paned notebook
	maBook <- ttknotebook(dlg)
	
	##notebook panes
	standFrame <- ttkframe(maBook, padding=8) 
	tkadd(maBook, standFrame, text='Standards')
	roiFrame <- ttkframe(maBook, padding=8) 
	tkadd(maBook, roiFrame, text='   ROIs   ')
	
	##add widgets to toplevel
	tkgrid(maBook, column=1, row=1, sticky='nsew', padx=c(6, 0), pady=c(6, 0))
	tkgrid(ttksizegrip(dlg), column=2, row=2, sticky='se')
	tkgrid.columnconfigure(dlg, 1, weight=1)
	tkgrid.rowconfigure(dlg, 1, weight=1)		
	
	####create widgets for standFrame
	##create file list box
	fileFrame <- ttklabelframe(standFrame, text='Files')
	fileList <- tclVar()
	fileNames <- names(fileFolder)
	if (!is.null(fileNames))
		tclObj(fileList) <- getTitles(fileNames)
	fileBox <- tklistbox(fileFrame, width=27, height=22, listvariable=fileList, 
			selectmode='extended', active='dotbox',	exportselection=FALSE, bg='white', 
			xscrollcommand=function(...) tkset(xscr, ...), 
			yscrollcommand=function(...) tkset(yscr, ...))
	xscr <- ttkscrollbar(fileFrame, orient='horizontal',
	...
	onDouble <- function(noROI=FALSE){
		...
		##assign current spectrum
		currentSpectrum <- usrFile
		myAssign('currentSpectrum', currentSpectrum, save.backup=FALSE)
		
		##parse file name
		if (length(grep('(', usrFile, fixed=TRUE)) && 
				length(grep('[', usrFile, fixed=TRUE))){
			splitName <- strsplit(usrFile, '(', fixed=TRUE)[[1]]
			compName <- basename(splitName[1])
			if (nzchar(compName)){
				splitName <- strsplit(splitName[2], ')', fixed=TRUE)[[1]]
				expName <- splitName[1]
				if (nzchar(expName)){
					compID <- strsplit(splitName[2], '[', fixed=TRUE)[[1]][2]
					compID <- strsplit(compID, ']', fixed=TRUE)[[1]][1]
					if (!nzchar(compID)){
						refresh()
						return(invisible())
					}
		...
		tclObj(nameVar) <- compName
		tclObj(expVar) <- expName
		tclObj(idVar) <- compID
		strFile <- paste('http://www.bmrb.wisc.edu/metabolomics/standards/', 
				compName, '/', 'nmr/', compID, '/', compID, '.str', sep='')
		tclObj(strVar) <- strFile
		
		##get peak list from STAR file
		tryCatch({
		...
		if (!noROI){
			roiPath <- file.path(dirname(currentSpectrum), 'roiRecord.roi')
			if (file.exists(roiPath)){
				roiRecord <- read.table(roiPath, head=TRUE, sep='\t', 
						stringsAsFactors=FALSE)
				roiTable <- roiRecord[grep(compID, roiRecord$Comp_ID), ]
				roiTable$ACTIVE <- rep(TRUE, nrow(roiTable))
				myAssign('roiTable', roiTable)
			}
		}
		refresh()
		...
		##configure GUI
		if (is.null(roiTable) || is.null(roiTable$Comp_ID) || 
				any(is.na(roiTable$Comp_ID)))
			tkconfigure(saveButton, state='disabled')
		else
			tkconfigure(saveButton, state='normal')
		tkwm.deiconify(dlg)
		tkfocus(fileBox)
	}
	tkbind(fileBox, '<Double-Button-1>', function(...) onDouble(...))
	
	##create add files button
	fileButtonFrame <- ttkframe(fileFrame)
	onAddFile <- function(){
		
		##open new files
		n <- length(fileNames)
		usrFiles <- sort(myOpen())
		if (!length(usrFiles) || !nzchar(usrFiles))
			return(invisible())
		newFiles <- silentOpen(usrFiles)
		if (!length(newFiles))
			err('You must select a valid UCSF (Sparky) format spectrum.')
		
		##add new files to list
		fileNames <<- c(fileNames, newFiles)
		tclObj(fileList) <- basename(fileNames)
		tkselection.clear(fileBox, 0, 'end')
		tkselection.set(fileBox, n)
		tcl(fileBox, 'see', n)
		onDouble()
		
		##alternate background colors of list items
		if (length(fileNames) > 2){
			for (i in seq(0, length(fileNames) - 1, 2))
				tkitemconfigure(fileBox, i, background='#ececff')
		}
	}
	addFileButton <- ttkbutton(fileButtonFrame, text='Add Files', width=12, 
			command=onAddFile)
	
	##create view library button (adds a directory of spectra)
	onViewLib <- function(){
		
		##open new files
		n <- length(fileNames)
		usrDir <- myDir()
		if (!length(usrDir) || !nzchar(usrDir))
			return(invisible())
		tkconfigure(dlg, cursor='watch')
		newFiles <- silentOpen(list.files(usrDir, full.names=TRUE))
	...
	viewLibButton <- ttkbutton(fileButtonFrame, text='View Library', width=12, 
			command=onViewLib)
	...
	onRemove <- function(){
		...
		##remove selection
		fileNames <<- fileNames[-usrSel]
		tclObj(fileList) <- basename(fileNames)
		tkselection.set(fileBox, length(fileNames) - 1)
		
		##close spectra		
		fc(usrFiles)
		if (!is.null(currentSpectrum) && currentSpectrum %in% usrFiles)
			onDouble()
	}
	removeButton <- ttkbutton(fileButtonFrame, text='Remove Selected', width=18, 
			command=onRemove)
	
	##configure strEntry based on text in other entry widgets
	configStrEntry <- function(){
		
		##get text from other entry widgets
		compName <- tclvalue(nameVar)
		if (!nzchar(compName))
			return(tclVar(TRUE))
		expName <- tclvalue(expVar)
		if (!nzchar(expName))
			return(tclVar(TRUE))
		compID <- tclvalue(idVar)
		if (!nzchar(compID))
			return(tclVar(TRUE))
	...
	##create compound name entry box
	infoFrame <- ttkframe(standFrame)
	nameFrame <- ttklabelframe(infoFrame, text='Compound Name', 
			padding=c(5, 0, 5, 4))
	nameVar <- tclVar('')
	nameEntry <- ttkentry(nameFrame, textvariable=nameVar, justify='left', 
			width=27, validate='all', validatecommand=configStrEntry)
	
	##create experiment entry box
	expFrame <- ttklabelframe(infoFrame, text='Experiment', padding=c(5, 0, 5, 4))
	expVar <- tclVar('')
	expEntry <- ttkentry(expFrame, textvariable=expVar, justify='left', width=27, 
			validate='all', validatecommand=configStrEntry)
	
	##create compound ID entry box
	idFrame <- ttklabelframe(infoFrame, text='Compound ID', padding=c(5, 0, 5, 4))
	idVar <- tclVar('')
	idEntry <- ttkentry(idFrame, textvariable=idVar, justify='left', width=27, 
			validate='all', validatecommand=configStrEntry)
	
	##create STAR file entry box
	strFrame <- ttklabelframe(infoFrame, text='NMR-STAR File', 
			padding=c(5, 0, 5, 4))
	strVar <- tclVar('')
	strEntry <- ttkentry(strFrame, textvariable=strVar, justify='left', width=27)
	
	##create peak display radiobuttons
	peakDispFrame <- ttkframe(strFrame)
	peakDispLabel <- ttklabel(peakDispFrame, text='Peak display:')
	peakDispVal <- tclVar(TRUE)
	onPeakOn <- function(){
		setGraphics(peak.disp=TRUE)
		pdisp()
		tkfocus(dlg)
	...
	setGraphics(peak.disp=TRUE)
	peakOnButton <- ttkradiobutton(peakDispFrame, variable=peakDispVal, 
			value=TRUE, text='On', command=onPeakOn)
	...
	peakOffButton <- ttkradiobutton(peakDispFrame, variable=peakDispVal, 
			value=FALSE, text='Off', command=onPeakOff)
	
	##create STAR file browse button
	strButtonFrame <- ttkframe(strFrame)
	onBrowse <- function(){
		
		##prompt for STAR file
		if (!length(fileNames))
			err('You must add a spectrum to the file list first.')
		strFile <- myOpen('str', list(str='NMR-STAR File'), 
				title='Select a STAR file', multi=FALSE)
		if (!length(strFile) || !nzchar(strFile))
			return(invisible())
		tclObj(strEntry) <- strFile
		
		##get peak list from STAR file
		tryCatch({
					strPeaks <- getStrPeaks(strFile)
					fileFolder[[currentSpectrum]]$peak.list <- strPeaks
					myAssign('fileFolder', fileFolder, save.backup=FALSE)
				}, error=function(er) err(paste('Could not find peak list for ', 
	...
	browseButton <- ttkbutton(strButtonFrame, text='Browse...', width=11, 
			command=onBrowse)
	...
	onDownload <- function(){
		if (!length(fileNames))
			err('You must add a spectrum to the file list first.')
		strFile <- tclvalue(strVar)
		
		##get peak list from STAR file
		tryCatch({
					strPeaks <- suppressWarnings(getStrPeaks(strFile))
					fileFolder[[currentSpectrum]]$peak.list <- strPeaks
					myAssign('fileFolder', fileFolder)
				}, error=function(er) err(paste('Could not find STAR file/peak list.', 
									'Make sure the URL and experiment name are valid.')))
	...
	downloadButton <- ttkbutton(strButtonFrame, text='Download', width=12, 
			command=onDownload)
	
	##create assign button
	assignFrame <- ttkframe(infoFrame)
	onAssign <- function(){
		...
		if (!length(fileNames))
			err('You must add a spectrum to the file list first.')
		if (is.null(roiTable))
			err('You must designate ROIs first.')
		
		##format ROIs
		compName <- tclvalue(nameVar)
		compID <- tclvalue(idVar)
		newTable <- roiTable
		newTable$Name <- rep(compName, length(newTable$Name))
		if (is.null(newTable$Assignment))
			newTable$Assignment <- rep(NA, length(newTable$Name))
		if (is.null(newTable$Peak_ID))
			newTable$Peak_ID <- rep(NA, length(newTable$Name))
		if (is.null(newTable$Comp_ID))
			newTable$Comp_ID <- rep(compID, length(newTable$Name))
		
		##exit if peak list is not provided
		if (is.null(fileFolder[[currentSpectrum]]$peak.list)){
			newTable <- newTable[order(newTable$Name), ]
			rownames(newTable) <- NULL
			myAssign('roiTable', orderROI(newTable))
			refresh()
			err('No peak list found, ROI assignment incomplete.')
		}
		
		##format peak list
		peakList <- fileFolder[[currentSpectrum]]$peak.list
		peakList$Assignment <- sapply(strsplit(peakList$Assignment, '_'), 
				function(x) x[2])
		
		##rename ROIs based on BMRB peak assignments
		assigned <- NULL
		for (i in 1:nrow(newTable)){
			for (j in 1:nrow(peakList)){
				
				##skip peaks that have already been assigned to an ROI
				if (peakList$Index[j] %in% assigned)
					next
				
				##assign peak if it falls within boundaries of ROI
				if (peakList$w1[j] > newTable$w1_upfield[i] && 
						peakList$w1[j] < newTable$w1_downfield[i] &&
						peakList$w2[j] > newTable$w2_upfield[i] && 
						peakList$w2[j] < newTable$w2_downfield[i]){
					if (is.na(newTable$Assignment[i]) || newTable$Assignment[i] == 'NA'){
						newTable$Assignment[i] <- peakList$Assignment[j]
						newTable$Peak_ID[i] <- j
					}else{
						newTable$Assignment[i] <- paste(unique(c(newTable$Assignment[i], 
												peakList$Assignment[j])), collapse=',')
						newTable$Peak_ID[i] <- paste(newTable$Peak_ID[i], j, sep=',')
					}
					assigned <- c(assigned, j)
				}
			}
			
			##denote unassigned ROIs as long range coupling
			if (is.na(newTable$Assignment[i]) || newTable$Assignment[i] == 'NA')
				newTable$Assignment[i] <- 'LRC'
		}
		newTable <- newTable[order(newTable$Name), ]
		rownames(newTable) <- NULL
		...
		##check for BMRB peaks that were not present in the ROI table
	...
	assignButton <- ttkbutton(assignFrame, text='Assign ROIs', width=11, 
			command=onAssign)
	
	##create assign text box
	assignText <- tktext(assignFrame, wrap='word', cursor='arrow', relief='flat', 
			font='TkDefaultFont', width=20, height=2, bg='SystemButtonFace')
	tkinsert(assignText, '1.0', 
			paste('Assign STAR peaks to matching ROIs'))
	tkconfigure(assignText, state='disabled')
	
	##create save button
	saveFrame <- ttkframe(infoFrame)
	onSave <- function(){
		...
		##prompt for library directory
		libPath <- myDir(title='Select library directory', parent=dlg)
		if (!length(libPath) || !nzchar(libPath))
			return(invisible())
		
		##add current spectrum to library
		tkconfigure(dlg, cursor='watch')
		addToLib(libPath, standName=tclvalue(nameVar), compID=tclvalue(idVar), 
				strPath=tclvalue(strVar))
		tkconfigure(dlg, cursor='arrow')
	}
	saveButton <- ttkbutton(saveFrame, text='Save...', width=11, state='disabled',
			command=onSave)
	
	##create save text box
	saveText <- tktext(saveFrame, wrap='word', cursor='arrow', relief='flat', 
			font='TkDefaultFont', width=20, height=2, bg='SystemButtonFace')
	tkinsert(saveText, '1.0', paste('Select a library and save the spectrum'))
	tkconfigure(saveText, state='disabled')
	
	
	####add widgets to standFrame
	##add widgets to fileFrame
	tkgrid(fileFrame, column=1, row=1, sticky='nswe', padx=c(0, 10))
	tkgrid(fileBox, column=1, row=1, sticky='nswe')
	tkgrid(yscr, column=2, row=1, sticky='ns')
	tkgrid(xscr, column=1, row=2, sticky='we')
	tkgrid(fileButtonFrame, column=1, columnspan=2, row=3, sticky='we', pady=6, 
			padx=6)
	tkgrid(addFileButton, column=1, row=1, pady=c(0, 4), padx=c(0, 8))
	tkgrid(viewLibButton, column=2, row=1, pady=c(0, 4))
	tkgrid(removeButton, column=1, columnspan=2, row=2)
	
	##make fileFrame stretch when window is resized
	tkgrid.columnconfigure(standFrame, 1, weight=1)
	tkgrid.rowconfigure(standFrame, 1, weight=10)
	tkgrid.columnconfigure(fileFrame, 1, weight=1)
	tkgrid.rowconfigure(fileFrame, 1, weight=1)
	
	##add widgets to infoFrame
	tkgrid(infoFrame, column=2, row=1, sticky='we', pady=4, padx=c(0, 4))
	tkgrid(nameFrame, column=1, row=1, sticky='we')
	tkgrid(nameEntry, column=1, row=1)
	
	tkgrid(expFrame, column=1, row=2, sticky='we', pady=c(10, 0))
	tkgrid(expEntry, column=1, row=1, sticky='we')
	
	tkgrid(idFrame, column=1, row=3, sticky='we', pady=c(10, 0))
	tkgrid(idEntry, column=1, row=1, sticky='we')
	
	tkgrid(strFrame, column=1, row=4, sticky='we', pady=c(10, 0))
	tkgrid(strEntry, column=1, row=1, pady=c(5, 2), sticky='e')
	tkgrid(strButtonFrame, column=1, row=2, pady=4, sticky='w')
	tkgrid(browseButton, column=1, row=1, padx=c(0, 6))
	tkgrid(downloadButton, column=2, row=1)
	tkgrid(peakDispFrame, column=1, row=3, pady=4, sticky='w')
	tkgrid(peakDispLabel, column=1, row=1)
	tkgrid(peakOnButton, column=2, row=1, padx=c(6, 8))
	tkgrid(peakOffButton, column=3, row=1)
	
	tkgrid(assignFrame, column=1, row=5, sticky='we', pady=c(30, 18))
	tkgrid(assignButton, column=1, row=1, padx=c(2, 6))
	tkgrid(assignText, column=2, row=1, sticky='we')
	
	tkgrid(saveFrame, column=1, row=6, sticky='we')
	tkgrid(saveButton, column=1, row=1, padx=c(2, 6))
	tkgrid(saveText, column=2, row=1, sticky='we')
	
	##make infoFrame stretch when window is resized
	tkgrid.rowconfigure(infoFrame, 0, weight=1)
	tkgrid.rowconfigure(infoFrame, 7, weight=1)
	
	##selects all rows Ctrl+A is pressed
	tkbind(dlg, '<Control-a>', function(...) 
				tkselection.set(fileBox, 0, 'end'))
	
	##updates file list whenever the mouse enters the GUI
	onMouse <- function(){
		reset(fileList, fileBox, fileNames)
		fileNames <<- names(fileFolder)
	}
	...
	####create widgets for roiFrame
	##create zoom radiobuttons
	zmFrame <- ttklabelframe(roiFrame, text='Zoom', padding=c(6, 0, 6, 6))
	zmRadioFrame <- ttkframe(zmFrame)
	rbVal <- tclVar('zoom')
	onZoom <- function(){
		tkconfigure(zmUpButton, text='In')
		tkconfigure(zmDownButton, text='Out')
		tkconfigure(zmLeftButton, state='disabled')
		tkconfigure(zmRightButton, state='disabled')
		tkfocus(zmArrowFrame)
	}
	zoomButton <- ttkradiobutton(zmRadioFrame, variable=rbVal, value='zoom', 
			text='zoom', command=onZoom)
	
	onScroll <- function(){
		tkconfigure(zmUpButton, text='^')
		tkconfigure(zmDownButton, text='v')
		tkconfigure(zmLeftButton, text='<', state='normal')
		tkconfigure(zmRightButton, text='>', state='normal')
		tkfocus(zmArrowFrame)
	}
	scrollButton <- ttkradiobutton(zmRadioFrame, variable=rbVal, value='scroll', 
			text='scroll', command=onScroll)
	
	##create zoom arrow buttons
	zmArrowFrame <- ttkframe(zmFrame)
	inc <- tclVar(20)
	onZmArrow <- function(type, direct, n){
		type=tclvalue(type)
		tryCatch({n <- as.numeric(tclvalue((inc)))
					if (n < 0)
						warning()
				}, warning = function(w){
					err('Increment value must be a positive number', parent=zmFrame)
				})
		if (type == 'scroll'){
			switch(direct, 'up'=pu(n), 
					'down'=pd(n), 
	...
	zmUpButton <- ttkbutton(zmArrowFrame, text='In', width=5, 
			command=function(...) onZmArrow(rbVal, 'up', inc))
	zmDownButton <- ttkbutton(zmArrowFrame, text='Out', width=5, 
			command=function(...)	onZmArrow(rbVal, 'down', inc))
	zmLeftButton <- ttkbutton(zmArrowFrame, text='<', width=4, state='disabled', 
			command=function(...) onZmArrow(rbVal, 'left', inc))
	zmRightButton <- ttkbutton(zmArrowFrame, text='>', width=4, state='disabled', 
			command=function(...) onZmArrow(rbVal, 'right', inc))
	zmIncEntry <- ttkentry(zmArrowFrame, textvariable=inc, width=5, 
			justify='center')
	
	##create other zoom buttons
	zmButtonFrame <- ttkframe(zmFrame)
	onFf <- function(){
		ff()
		tkfocus(zmFrame)
		tkwm.deiconify(dlg)
	...
	ffButton <- ttkbutton(zmButtonFrame, text='Full', width=8, command=onFf)
	...
	zcButton <- ttkbutton(zmButtonFrame, text='Center', width=8, command=onZc)
	...
	zzButton <- ttkbutton(zmButtonFrame, text='Hand', width=8, command=onZz)
	...
	pzButton <- ttkbutton(zmButtonFrame, text='Point', width=8, command=onPz)
	...
	zpButton <- ttkbutton(zmButtonFrame, text='Prev', width=8, command=onZp)
	...
	locButton <- ttkbutton(zmButtonFrame, text='Get shifts', width=8, 
			command=onLoc)	
	
	##create minimum countour threshold slider
	threshFrame <- ttklabelframe(roiFrame, text='Contour Threshold', 
			padding=c(6, 0, 6, 6))
	if (!is.null(currentSpectrum))
		clevel <- fileFolder[[currentSpectrum]]$graphics.par$clevel
	else
	...
	threshSlider <- tkscale(threshFrame, from=0, to=threshMax, variable=threshVal, 
			orient='horizontal', showvalue=T, tickinterval=threshMax, width=12, 
			length=285, resolution=threshRes, 
			bg=as.character(tkcget(dlg, '-background')))
	onThreshSlider <- function(){
		if (is.null(currentSpectrum))
			return(invisible())
		tkconfigure(threshSlider, fg='black')
		setGraphics(currentSpectrum, clevel=as.numeric(tclObj(threshVal)), 
				refresh.graphics=TRUE)
		tkfocus(dlg)
		tkwm.deiconify(dlg)
		bringFocus()
	...
	threshDecButton <- ttkbutton(threshFrame, text='<', width=4, command=onMaxDec)
	...
	threshIncButton <- ttkbutton(threshFrame, text='>', width=4, command=onMaxInc)
	
	##reconfigures threshold slider to match setting for currentSpectrum
	configThresh <- function(){
		if (is.null(currentSpectrum))
			return(invisible())
		clevel <- fileFolder[[currentSpectrum]]$graphics.par$clevel
		if (clevel > 25){
	...
	##create auto roi button
	roiCreateFrame <- ttklabelframe(roiFrame, text='Create ROIs', 
			padding=c(6, 0, 6, 6))
	onAutoRoi <- function(){
		ra()
		tkfocus(dlg)
	...
	roiAutoButton <- ttkbutton(roiCreateFrame, text='Auto ROI', width=12, 
			command=onAutoRoi)
	
	##create new roi button
	onSemiAuto <- function(){
		
		## Set defaults normally provided as parameters
		if (is.null(currentSpectrum))
			return(invisible())
		w1Delta <- globalSettings$roi.w1
		w2Delta <- globalSettings$roi.w2
		p <- globalSettings$roi.pad
		noiseFilt <- globalSettings$roi.noiseFilt
		...
		while(TRUE){
			...
			legend("topleft", c('LEFT CLICK FOR ROI','RIGHT CLICK TO EXIT'), 
					pch=NULL, bty='n', 
					text.col=fileFolder[[currentSpectrum]]$graphics.par$fg)
			par(font=op)
			...
			## Find current spectrum and establish chemical shift range
			p <- p / 100 + .5
			filePar <- fileFolder[[wc()]]$file.par
			res <- (filePar$downfield_ppm - filePar$upfield_ppm) / filePar$matrix_size	
			
			## Calculate the granularity constants
			gran <- NULL
			for (i in 1:2){
				if (filePar$nucleus[i] == '1H')
					gran <- c(gran, .05)
				else
					gran <- c(gran, .5)
			}
			...
			## Generate a fancy peak list 
			peakList <- peakPick(w1Range=xy$y, w2Range=xy$x, append=FALSE, 
					internal=TRUE, fancy=TRUE, w1Gran=w1Gran, w2Gran=w2Gran, 
					noiseFilt=noiseFilt)	
			if (is.null(peakList)){
				cat('No peaks were detected \n')
			...
			## Translate fancy peak list to roi table
			peakList <- data.frame('ROI', max(peakList$w2 + peakList$w2D), 
					min(peakList$w2 - peakList$w2D), max(peakList$w1 + peakList$w1D), 
					min(peakList$w1 - peakList$w1D), TRUE, 2, 
					fileFolder[[currentSpectrum]]$graphics.par$clevel, 
					stringsAsFactors=FALSE)
			names(peakList) <- c('Name', 'w2_downfield', 'w2_upfield', 'w1_downfield', 
					'w1_upfield', 'ACTIVE', 'nDim', 'clevel')
			peakList$clevel <- fileFolder[[currentSpectrum]]$graphics.par$clevel
			newRois <- rbind(newRois, peakList)
			showRoi(rTable=newRois)
		}
		if (is.null(newRois)){
			refresh()
			showGui()
		...
		## Add new ROI(s) to roiTable
		if (!is.null(roiTable$Name[1]))
			roiTable <- merge(roiTable, newRois, all=TRUE)
		else
			roiTable <- rbind(roiTable, newRois)
		roiTable <- roiTable[order(roiTable$w1_upfield, roiTable$w2_upfield), ]
		roiTable <- orderROI(roiTable)
		myAssign('roiTable', roiTable)	
		refresh()
		showGui()
		tkfocus(dlg)
	...
	roiSemiButton <- ttkbutton(roiCreateFrame, text='Semi-Auto', width=12, 
			command=onSemiAuto)
	
	##create ROI padding button
	onRoiPad <- function(){
		usrPad <- myDialog('Enter padding amount (%):', defaultSettings$roi.pad, 
				'ROI Padding', parent=dlg)
		if (is.null(usrPad) || !nchar(usrPad))
			return(invisible())
		usrPad <- suppressWarnings(as.numeric(usrPad))
		if (!is.na(usrPad))
			setGraphics(roi.pad=usrPad)
	}
	roiPadButton <- ttkbutton(roiCreateFrame, text='Padding', width=12, 
			command=onRoiPad)
	
	##create noise filter radio buttons
	setGraphics(roi.noiseFilt=0)
	onRoiFilt <- function(){
		setGraphics(roi.noiseFilt=as.numeric(tclvalue(roiFiltVar)))
	}
	roiFiltVar <- tclVar(globalSettings$roi.noiseFilt)
	strongButton <- ttkradiobutton(roiCreateFrame, variable=roiFiltVar, value=2, 
			text='Strong filter', command=onRoiFilt)
	mildButton <- ttkradiobutton(roiCreateFrame, variable=roiFiltVar, value=1, 
			text='Mild filter', command=onRoiFilt)
	peakOffButton <- ttkradiobutton(roiCreateFrame, variable=roiFiltVar, value=0, 
			text='No noise filter', command=onRoiFilt)
	
	##create select all button
	roiSelFrame <- ttklabelframe(roiFrame, text='Select ROIs', 
			padding=c(6, 0, 6, 6))
	onSelAll <- function(){
		if (is.null(currentSpectrum))
			return(invisible())
	...
	selAllButton <- ttkbutton(roiSelFrame, text='All', width=9, command=onSelAll)
	...
	selNoneButton <- ttkbutton(roiSelFrame, text='None', width=9, command=onSelNone)
	...
	selListButton <- ttkbutton(roiSelFrame, text='List', width=9, 
			command=onSelList)
	
	##create single button
	selPlotLabel <- ttklabel(roiSelFrame, text='Main plot:')
	onSelSingle <- function(){
		rs(2)
		tkfocus(dlg)
		tkwm.deiconify(dlg)
		bringFocus()
	}
	singleButton <- ttkbutton(roiSelFrame, text='Single', width=9, 
			command=onSelSingle)
	...
	selRegButton <- ttkbutton(roiSelFrame, text='Region', width=9, 
			command=onSelReg)
	
	##create ROI adjust radiobuttons
	roiEditFrame <- ttklabelframe(roiFrame, text='Edit ROIs', 
			padding=c(6, 0, 6, 6))
	adjTypeFrame <- ttkframe(roiEditFrame, padding=3)
	adjRbVal <- tclVar('move')
	roiMoveButton <- ttkradiobutton(adjTypeFrame, variable=adjRbVal, value='move', 
			text='move')
	roiExpandButton <- ttkradiobutton(adjTypeFrame, variable=adjRbVal, 
			value='expand', text='expand')
	roiContractButton <- ttkradiobutton(adjTypeFrame, variable=adjRbVal, 
			value='contract', text='contract')
	
	##create ROI arrow buttons
	roiArrowFrame <- ttkframe(roiEditFrame, padding=3)
	amount <- tclVar(20)
	onRoiArrow <- function(type, direct, n){
		if (tclvalue(incRbVal) == 'percent')
			ppmInc <- FALSE
		else
			ppmInc <- TRUE
	...
	roiUpButton <- ttkbutton(roiArrowFrame, text='^', width=5, 
			command=function(...)	onRoiArrow(adjRbVal, 'up', amount))
	roiDownButton <- ttkbutton(roiArrowFrame, text='v', width=5, 
			command=function(...) onRoiArrow(adjRbVal, 'down', amount))
	roiLeftButton <- ttkbutton(roiArrowFrame, text='<', width=4, 
			command=function(...) onRoiArrow(adjRbVal, 'left', amount))
	roiRightButton <- ttkbutton(roiArrowFrame, text='>', width=4, 
			command=function(...) onRoiArrow(adjRbVal, 'right', amount))
	roiIncEntry <- ttkentry(roiArrowFrame, textvariable=amount, width=5, 
			justify='center')
	
	##create adjustment increment radiobuttons
	roiIncFrame <- ttkframe(roiEditFrame)
	incRbVal <- tclVar('percent')
	roiIncLabel <- ttklabel(roiIncFrame, text='Increment:')
	roiPercentButton <- ttkradiobutton(roiIncFrame, variable=incRbVal, 
			value='percent', text='percent')
	roiPpmButton <- ttkradiobutton(roiIncFrame, variable=incRbVal, value='ppm', 
			text='ppm')
	
	##create roi center button
	roiEditButFrame <- ttkframe(roiEditFrame)
	onCenterRoi <- function(){
		rc()
		tkfocus(dlg)
		tkwm.deiconify(dlg)
		bringFocus()
	}
	roiCenterButton <- ttkbutton(roiEditButFrame, text='Center Active', width=12, 
			command=onCenterRoi)
	...
	roiDelButton <- ttkbutton(roiEditButFrame, text='Delete', width=12, 
			command=onDelRoi)
	
	##create edit table button
	editTableButton <- ttkbutton(roiEditButFrame, text='Edit Table', width=12, 
			command=function() re())
	
	##creates undo button
	undoFrame <- ttkframe(roiFrame)
	onUndo<- function(){
		ud()
		tkfocus(dlg)
		tkwm.deiconify(dlg)
	...
	undoButton <- ttkbutton(undoFrame, text='Undo', width=11, command=onUndo)
	...
	redoButton <- ttkbutton(undoFrame, text='Redo', width=11, command=onRedo)
	
	####add widgets to roiFrame
	##add widgets to zmFrame
	tkgrid(zmFrame, column=1, columnspan=2, row=1, sticky='we', pady=c(4, 8))
	tkgrid(zmRadioFrame, column=1, row=1)
	tkgrid(zoomButton, column=1, row=1, pady=c(0, 16))
	tkgrid(scrollButton, column=1, row=2)
	
	tkgrid(zmArrowFrame, column=2, row=1, padx=12)
	tkgrid(zmUpButton, column=2, row=1, sticky='s')
	tkgrid(zmLeftButton, column=1, row=2, sticky='e')
	tkgrid(zmIncEntry, column=2, row=2)
	tkgrid(zmRightButton, column=3, row=2, sticky='w')
	tkgrid(zmDownButton, column=2, row=3, sticky='n')
	
	tkgrid(zmButtonFrame, column=3, row=1, padx=c(4, 0))
	tkgrid(ffButton, column=1, row=1, pady=c(0, 6))
	tkgrid(zpButton, column=2, row=1, padx=4, pady=c(0, 6))
	tkgrid(zcButton, column=3, row=1, pady=c(0, 6))
	tkgrid(pzButton, column=1, row=2)
	tkgrid(zzButton, column=2, row=2, padx=4)
	tkgrid(locButton, column=3, row=2)
	
	##add widgets to threshFrame
	tkgrid(threshFrame, columnspan=2, column=1, row=2, sticky='we', pady=c(0, 8))
	tkgrid(threshSlider, column=1, row=1)
	tkgrid(threshDecButton, column=2, row=1, padx=c(3, 2))
	tkgrid(threshIncButton, column=3, row=1)
	
	## Add widgets to roiCreateFrame
	tkgrid(roiCreateFrame, column=1, row=3, sticky='w', pady=c(0, 8), 
			padx=c(0, 8))
	tkgrid(roiAutoButton, row=1, column=1)
	tkgrid(roiSemiButton, row=2, column=1, pady=4)
	tkgrid(roiPadButton, row=3, column=1)
	
	tkgrid(strongButton, column=2, row=1, padx=c(12, 0), sticky='w')
	tkgrid(mildButton, column=2, row=2, pady=2, padx=c(12, 0), sticky='w')
	tkgrid(peakOffButton, column=2, row=3, padx=c(12, 0), sticky='w')
	
	##add widgets to roiSelFrame
	tkgrid(roiSelFrame, row=3, column=2, sticky='e', pady=c(0, 8))
	tkgrid(selAllButton, row=1, column=1, padx=c(0, 6))
	tkgrid(selNoneButton, row=2, column=1, padx=c(0, 6), pady=3)
	tkgrid(selListButton, row=3, column=1, padx=c(0, 6))
	tkgrid(selPlotLabel, row=1, column=2, sticky='w', padx=6)
	tkgrid(singleButton, row=2, column=2, padx=c(10, 0), pady=3, sticky='w')
	tkgrid(selRegButton, row=3, column=2, padx=c(10, 0), sticky='w')
	
	##add widgets to roiEditFrame
	tkgrid(roiEditFrame, columnspan=2, column=1, row=4, sticky='we', 
			pady=c(0, 12))
	tkgrid(adjTypeFrame, column=1, row=1)
	tkgrid(roiMoveButton, column=1, row=1, sticky='w')
	tkgrid(roiExpandButton, column=1, row=2, pady=8, sticky='w')
	tkgrid(roiContractButton, column=1, row=3, sticky='w')
	
	tkgrid(roiArrowFrame, column=2, row=1, padx=2)
	tkgrid(roiUpButton, column=2, row=1, sticky='s', pady=c(3, 0))
	tkgrid(roiLeftButton, column=1, row=2, sticky='e')
	tkgrid(roiIncEntry, column=2, row=2)
	tkgrid(roiRightButton, column=3, row=2, sticky='w')
	tkgrid(roiDownButton, column=2, row=3, sticky='n', pady=c(0, 4))
	
	tkgrid(roiIncFrame, column=3, row=1, padx=c(0, 25))
	tkgrid(roiIncLabel, column=1, row=1, sticky='w')
	tkgrid(roiPercentButton, column=1, row=2, padx=c(12, 0), pady=8, sticky='w')
	tkgrid(roiPpmButton, column=1, row=3, padx=c(12, 0), sticky='w')
	
	tkgrid(roiEditButFrame, column=4, row=1, pady=6, padx=c(0, 0))
	tkgrid(roiCenterButton, row=1, column=1)
	tkgrid(roiDelButton, row=2, column=1, pady=4)
	tkgrid(editTableButton, row=3, column=1)
	
	##add widgets to undoFrame
	tkgrid(undoFrame, columnspan=2, column=1, row=5, pady=c(0, 4))
	tkgrid(undoButton, row=1, column=1, padx=20)
	tkgrid(redoButton, row=1, column=2)
	
	##populate entries in the standard tab if the file list is not empty
	n <- length(fileNames)
	if (n){
		tkselection.set(fileBox, wc() - 1)
		onDouble(TRUE)
	...
	onEnter <- function(){
		...
		if (focus == '.ma.1.1.1.1')
			...
			try(tkinvoke(focus), silent=TRUE)
	}
	tkbind(dlg, '<Return>', onEnter)
...
############################## BMRB_libs.R #####################################
#
#for (i in list.files('C:/BMRB Data', pattern='.sf', full.names=TRUE)){
#	currId <- strsplit(i, '[', fixed=TRUE)[[1]][2]
#	currId <- strsplit(currId, ']', fixed=TRUE)[[1]][1]
#	roiTable <- read.table('C:/BMRB Data/roiRecord.roi',	head=TRUE, sep='\t', 
#			stringsAsFactors=FALSE)
#	roiTable <- roiTable[grep(currId, roiTable$Comp_ID), ]
#	createRsd(i, rois=roiTable)
#}


##Check BMRB library for updates and assignments
checkBMRB <- function(assignments=TRUE, allUpdates=FALSE){
	
	##define some local variables
	if (allUpdates){
		assignments <- FALSE
		libUpdates <- c('BMRB_Name', 'ID', 'Update', 'Detail')
	}
	
	##read in Release.csv from BMRB FTP site
	release <- read.table(paste('http://www.bmrb.wisc.edu/ftp/pub/bmrb', 
					'/relational_tables/metabolomics/Release.csv', sep=''), head=TRUE, 
			sep=',',	stringsAsFactors=FALSE)
	release$Date <- as.Date(release$Date)
	
	##read in Assigned_peak_chem_shift.csv from BMRB FTP site
	if (assignments)
		assignedPeaks <- read.table(paste('http://www.bmrb.wisc.edu/ftp/pub/bmrb', 
						'/relational_tables/metabolomics/Assigned_peak_chem_shift.csv', 
						sep=''), head=TRUE,	sep=',',	stringsAsFactors=FALSE)
	
	##read in library contents
	libInfo <- read.table('H:/R files/docs/BMRB_library.tab', head=TRUE, sep='\t',	
			stringsAsFactors=FALSE)
	
	##check BMRB relational tables, one compound at a time
	for (i in seq_along(libInfo$Compound)){
		
		##find current compound in Release.csv
		matches <- grep(libInfo$ID[i], release$Entry_ID, fixed=TRUE)
		if (!length(matches))
			next
		...
		if (allUpdates){
			for (j in matches){
				currUpdate <- c(libInfo$BMRB_Name[i], libInfo$ID[i], 
						as.character(release$Date[j]), release$Detail[j])
				libUpdates <- rbind(libUpdates, currUpdate)
			}
			next
		}
		
		##get most recent update 
		updated <- matches[which.max(release$Date[matches])]
		libInfo$Last_Update[i] <- libInfo$Updated[i]
		libInfo$Updated[i] <- as.character(release$Date[updated])
		libInfo$Detail[i] <- release$Detail[updated]
		if (is.na(libInfo$Last_Update[i]))
			libInfo$Last_Update[i] <- libInfo$Updated[i]
		
		##check for assignments
		if (assignments){
			libInfo$Prev_Assigned[i] <- libInfo$Assigned[i]
			if (length(grep(libInfo$ID[i], assignedPeaks$Entry_ID, fixed=TRUE)))
				libInfo$Assigned[i] <- 'yes'
			else
				libInfo$Assigned[i] <- 'no'
		}
	}
	
	##write table out to file
	if (allUpdates)
		write.table(libUpdates, 'H:/R files/docs/BMRB_library_updates.tab', 
				quote=FALSE, sep='\t', row.names=FALSE, col.names=FALSE)
	else	
		write.table(libInfo, 'H:/R files/docs/BMRB_library.tab', quote=FALSE, 
				sep='\t', row.names=FALSE)
}


##convert a Bruker spectrum without using a GUI
conBruk <- function(inDir, nDim, outFile){
	...
	if (nDim == 1)
		...
		pattern <- "^2rr$"
	pDataPaths <- list.files(inDir, recursive=TRUE, full.names=TRUE, 
			pattern=pattern)[1]
	rmPaths <- NULL
	for (i in seq_along(pDataPaths)){
		if (!length(grep('/pdata/', pDataPaths[i])))
	...
	if (!is.null(rmPaths))
		pDataPaths <- pDataPaths[-rmPaths]
	if (!length(pDataPaths))
		return('Could not find Bruker format processed data file ("1r" or "2rr")')
	pDataPath <- pDataPaths[1]
	
	##get acquisition parameters
	acquPar <- parseAcqus( strsplit(pDataPath, '/pdata')[[1]][1])
	if (!length(acquPar) || length(grep('2rr$', pDataPath)) && 
			nrow(acquPar) != 2)
		return(paste('Can not convert "', pDataPath, '",\n  Could not find ',
						'acquisition parameter files ("acqu2" or "acqu2s")\n', sep=''))
	
	##get processing parameters
	procPar <- parseProcs(dirname(pDataPath))
	if (!length(procPar) || length(grep('2rr$', pDataPath)) && 
			nrow(procPar) != 2)
		return(paste('Can not convert "', pDataPath, '",\n  Could not find ',
						'processing parameter files ("proc2" or "proc2s")\n', sep=''))
	...
	##convert 1D spectra
	if (length(grep('1r$', pDataPath))){
		if (nrow(acquPar) == 2)
			acquPar <- acquPar[1, ]
		if (nrow(procPar) == 2)
			procPar <- procPar[1, ]
		outMsg <- bruker1D(pDataPath, outFile, acquPar$NUC1, acquPar$SFO1, 
				procPar$BYTORDP, procPar$NC_proc, procPar$OFFSET, procPar$SF, 
				procPar$SI, procPar$SW_p, FALSE)
		
	}else{
		
		##convert 2D spectra
		outMsg <- bruker2D(pDataPath, outFile, acquPar$NUC1, acquPar$SFO1, 
				procPar$BYTORDP[1],	procPar$NC_proc[1], procPar$OFFSET, procPar$SF, 
				procPar$SI,	procPar$SW_p, procPar$XDIM)
	}
...
##convert a NMRPipe spectrum without using a GUI
conPipe <- function(inDir, outFile){
	
	##get processed data path
	pDataPath <- sort(unique(c(list.files(inDir, recursive=TRUE, 
									full.names=TRUE, pattern=".dat$"), list.files(inDir, 
...
##download, unpack, and convert the Bruker/Pipe spectra for a given experiment
##	type from the BMRB
downloadTar <- function(compName, id, outDir, exp='HSQC', skipExist=TRUE){
	
	##read str file  _Spectral_peak_list.Experiment_ID  
	strLoc <- paste('http://www.bmrb.wisc.edu/ftp/pub/bmrb/metabolomics/',
			'standards/', compName, '/nmr/', id, '/', id, '.str', sep='')
	strFile <- readLines(strLoc)
	
	##get experiment directories table
	expStart <- grep('_Experiment_file.Experiment_list_ID', strFile) + 2
	expEnd <- grep(' stop_', strFile)
	expEnd <- expEnd[which(grep(' stop_', strFile) > expStart)][1] - 1
	expDirs <- sapply(strsplit(strFile[expStart:expEnd], ' +'), function(x) x[3])
	
	##filter out unneeded directories
	pngs <- grep('spectra_png', expDirs)
	if (length(pngs))
		expDirs <- expDirs[-pngs]
	peaks <- grep('peak_lists', expDirs)
	if (length(peaks))
		expDirs <- expDirs[-peaks]
	expDirs <- sub('*', '', expDirs, fixed=TRUE)
	expDirs <- expDirs[grep(exp, expDirs, fixed=TRUE)]
	if (!length(expDirs))
		stop('Could not download tar for ', compName, '(', id, ')', 
				'\nSpecified experiment does not exist', sep='') 
	expDirs <- sapply(strsplit(expDirs, '/', fixed='TRUE'), function(x) 
				paste(x, collapse='/'))
	
	##download and convert each spectrum one at a time
	contentsPath <- file.path(outDir, 'contents.txt')
	outMsgs <- NULL
	for (i in expDirs){
		
		##don't bother converting any files that already exist in the output dir
		if (skipExist){
			outName <- paste(outDir, '/', compName, '(', basename(i), ')', '[', id, 
					']', '.sf', sep='')
		...
		##unpack tar file
		tarLoc <- paste('http://www.bmrb.wisc.edu/ftp/pub/bmrb/metabolomics/', i, 
				'.tar', sep='')
		cat('Unpacking:\n ', tarLoc, '\n')
		flush.console()
		tarCon <- file(tarLoc, 'rb')
		tmp <- try(suppressWarnings(untar(tarCon, exdir='~')))
		close(tarCon)
		if (inherits(tmp, 'try-error')){
			outMsgs <- c(outMsgs, format(tmp))
			next
		}
		
		##get directory for unpacked spectrum
		dataDir <- file.path('~', basename(i))
		if (!file.exists(dataDir))
			return(paste('Could not unpack', tarLoc))
		
		##convert spectrum to ucsf format
		if (length(grep('TOCSY|HSQC|HMBC|COSY', exp)))
			nDim <- 2
		else
			nDim <- 1
		outMsg <- conBruk(dataDir, nDim, outName)
		if (!length(grep('Successful', outMsg)))
			outMsg <- conPipe(dataDir, outName)
		else
			write.table(t(c(compName, id, basename(i), format(Sys.time(), 
											'%m/%d/%Y'))), contentsPath, quote=FALSE, sep='\t', 
					row.names=FALSE, col.names=FALSE, append=TRUE)
		unlink(path.expand(dataDir), TRUE)
		
		##print message
		outMsg <- paste(outMsg, '\n\n', sep='')
		cat(outMsg)
		outMsgs <- c(outMsgs, outMsg)
	}
...
##update local BMRB library
addToLocal <- function(compName){
	
	libContents <- read.table('H:/R files/docs/BMRB_library.tab',	head=TRUE, 
			sep='\t', stringsAsFactors=FALSE)
	if (!missing(compName))
		libContents <- libContents[match(compName, libContents$BMRB_Name), ]
	for (i in seq_along(libContents$Compound)){
		if (libContents$BMRB_Name[i] == '?')
			next
		result <- try(downloadTar(libContents$BMRB_Name[i], libContents$ID[i], 
						outDir='H:/R files/new_BMRB_spec'), silent=TRUE)
...
##create an RSD standard library from a given ROI record
##sourceDir - character string; directory containing the source spectra for the 
##	RSD files
##roiRec - character string; full path to an ROI record file
##outLoc  - character string; output file path for library (a directory)
createLib <- function(sourceDir, roiRec, outDir){
	
	##get source directory if not provided
	if (missing(sourceDir)){
		sourceDir <- myDir(title='Select a source directory:')
		if (!nchar(sourceDir))
			return(invisible())
	}
	
	##get path to ROI record if not provided
	if (missing(roiRec)){
		roiRec <- myOpen(title='Select an ROI Record:')
		if (!nchar(roiRec))
			return(invisible())
	}
	...
	if (missing(outDir)){
		outDir <- myDir(title='Select an output directory:')
		if (!nchar(outDir))
			return(invisible())
	}
	
	##read in the ROI record
	roiRecord <- read.table(roiRec,	head=TRUE, sep='\t', stringsAsFactors=FALSE)
	
	##create RSD files, one spectrum at a time
	missSpec <- NULL
	compNames <- unique(lapply(strsplit(roiRecord$Name, '.', fixed=TRUE), 
					function(x) x[1]))
	for (i in compNames){
		
		##get the path to the source spectrum
		dirFiles <- list.files(sourceDir, full.names=TRUE)
		inFile <- dirFiles[grep(i, dirFiles, fixed=TRUE)]
		if (length(grep('bmse', inFile))){
			inFile <- inFile[grep('HSQC', inFile, fixed=TRUE)]
			inFile <- inFile[grep('.sf', inFile, fixed=TRUE)]
		}
		inFile <- inFile[1]
		
		##skip file if no spectrum is present
		if (!length(inFile) || is.na(inFile)){
			missSpec <- c(missSpec, i)
			next
		}
		
		##format the output path
		outFile <- paste(unlist(strsplit(basename(inFile), '.', fixed=TRUE))[1], 
				'rsd', sep='.')
		outFile <- file.path(outDir, outFile)
		
		##get the current set of ROIs
		currRois <- roiRecord[grep(i, roiRecord$Name), ]
	...
	if (!is.null(missSpec)){
		cat('\nCould not find source spectra for the following compounds:\n')
		for (i in missSpec){
			cat(i)
			cat('\n')
		}
...
#createLib(sourceDir='C:/Users/Work/Desktop/roiRecord spec', 
#		roiRec='C:/Users/Work/Desktop/roiRecord.roi', 
#		outDir='C:/Users/Work/Desktop/New Folder')

##download and convert BMRB spectra listed in the BMRB's Entry.csv file
getBmrbSpec <- function(outDir='C:/BMRB HSQCs', exp='HSQC', 
		skipExist=TRUE, usrSelect = TRUE){
	
	##get list of spectra from BMRB using the Entry.csv file
	bmrbList <- read.csv(paste('http://www.bmrb.wisc.edu/ftp/pub/bmrb/', 
					'relational_tables/metabolomics/Entry.csv', sep=''), 
			colClasses='character')
	ids <- unique(bmrbList$ID)
	
	if( usrSelect ){
		ids <- mySelect(ids, multiple = TRUE, title = "Select Spectrum")
		
		if( length(ids) == 1 && ids == "" )
			stop("No spectra selected")
	}
	
	
	errors <- list()
	
	##loop through the compounds listed in the Experiment file and convert them
	for (i in seq_along(ids)){
		
		##get the BMRB directory name, which we'll use as the name for the compound
		compName <- bmrbList$BMRB_internal_directory_name[match(ids[i], 
						bmrbList$ID)]
		
		##no directory listed for this compound, save error and move on
		if (!length(compName) || !nzchar(compName) || compName == ' ' || 
				compName == '.'){
			errors[[i]] <- 'Directory for specified compound does not exist.'
			names(errors)[i] <- bmrbList$Entry_title[match(ids[i], bmrbList$ID)]
			next
		}
		
		##convert the spectrum and save any errors
		error <- try(downloadTar(compName, ids[i], outDir, exp, skipExist), 
				silent=TRUE)
		if (class(error) == 'try-error'){
			n <- length(errors) + 1
			errors[[n]] <- format(error)
			names(errors)[n] <- bmrbList$Entry_title[match(ids[i], bmrbList$Entry_ID)]
		}
		
		##free up memory after we're done
		gc(FALSE)
	}
...
##save metadata for one or more RSD files
saveMeta <- function(exp='HSQC'){
	
	##get RSD files
	usrFiles <- myOpen(title='Select RSD files')
	if (!length(usrFiles) || !nchar(usrFiles[1]))
		return(invisible())
	rsdNames <- basename(usrFiles)
	ids <- sapply(strsplit(rsdNames, '[', fixed=TRUE), function(x) 
				strsplit(x[2], ']')[[1]][1])
	
	##get file containing metadata for BMRB compounds
	expRecPath <- myOpen(title='Select experiment record', multiple=FALSE)
	if (!nchar(expRecPath))
		return(invisible())
	expRec <- read.csv(expRecPath, sep=',', quote='', row.names=NULL, 
			comment.char='')
	expRec <- expRec[grep(exp, expRec$Expt_name, fixed=TRUE), ]
	colNames <- c('Entry_ID',	'Entry_title', 'BMRB_internal_dir', 'Expt_name', 
			'Expt_ID', 'Field_strength', 'Sample_conc', 'Solvent', 'Buffer', 'cs_ref', 
			'pH', 'Temperature')
	
	##get data for specified files one at a time
	metadata <- NULL
	for (id in ids){
	...
	metadata$RSD_name <- rsdNames
	metadata$Updated <- rep(format(Sys.time(), "%Y%m%d"), length(rsdNames))
	
	##wrtite metadata out to file
	outFile <- file.path(dirname(usrFiles[1]), 'metadata.txt') 
	write.table(metadata, outFile, append=TRUE, quote=FALSE, sep='\t', 
			row.names=FALSE)
	
	return(outFile)
}

##create a hash file from a set of untouched UCSF spectra
createLibHash <- function(){
	
	##interpolate spectra
	inPaths <- myOpen(title='Select input UCSF spectra')
	interpDir <- myDir(title='Select output directory for interp spectra')
	outPaths <- paste(interpDir, '/interp_', basename(inPaths), sep='')
	interpSpec(outPath=outPaths, inPath=inPaths)
	
	##smooth spectra to hash resolution
	smoothDir <- myDir(title='Select output directory for smoothed spectra')
	batchSmooth(dirname(outPaths[1]), smoothDir)
	
	##create hash file
	inPaths <- list.files(smoothDir, pattern=('sf|ucsf'), full.names=TRUE)
	outPath <- file.path(dirname(dirname(smoothDir)), 'hash.ucsf')
	hash <- mathNMR(inPath=inPaths, outPath=outPath, normFun='threshold')
}

##create full index of spectra in RSD libraries
buildLibIndex <- function(){
	libDir <- myDir(title='Select library directory')
	rsdPaths <- list.files(libDir, pattern='rsd', full.names=TRUE, recursive=TRUE)
	Name <- basename(rsdPaths)
	Library <- basename(dirname(rsdPaths))
	Updated <- rep(format(Sys.time(), "%Y%m%d"), length(rsdPaths))
	write.table(cbind(Name, Library, Updated), file.path(libDir, 'index.txt'), 
